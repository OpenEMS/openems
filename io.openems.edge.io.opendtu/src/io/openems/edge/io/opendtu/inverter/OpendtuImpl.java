...
public class OpendtuImpl extends AbstractOpenemsComponent
	        ...
	        for (var inverterElem : inverters) {
	            ...
	            if (inverter.has("AC")) {
	                ...
	                if (acData.has("0")) {
	                    ...
	                    Integer power = extractValueAsInteger(ac0Data, "Power");
	                    Integer voltage = extractValueAsInteger(ac0Data, "Voltage") * 1000;
	                    Integer current = extractValueAsInteger(ac0Data, "Current") * 1000;
	                    Integer frequency = extractValueAsInteger(ac0Data, "Frequency");

	                    setValuesForInverter(serial, voltage, current, frequency, power);
	...
	private Integer extractValueAsInteger(JsonObject jsonObject, String key) {
	    if (jsonObject.has(key) && jsonObject.get(key).isJsonObject()) {
	        ...
	        if (data.has("v") && data.get("v").isJsonPrimitive()) {
	            return (int) data.get("v").getAsFloat();
	        }
	    }
	    return 0; // or handle this case as needed
	}

	private void setValuesForInverter(String serial, Integer voltage, Integer current, Integer frequency, Integer power) {
