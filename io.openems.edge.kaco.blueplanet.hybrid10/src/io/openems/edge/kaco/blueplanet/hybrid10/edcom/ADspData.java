...
public abstract class ADspData {
    ...
    private static final String PermList[][] = {
        { "g_sync.u_l_rms[0]",                  "1" , "5" },
        { "g_sync.u_l_rms[1]",                  "1" , "5" },
        { "g_sync.u_l_rms[2]",                  "1" , "5" },
        { "g_sync.u_sg_avg[0]",                 "1" , "5" },
        { "g_sync.u_sg_avg[1]",                 "1" , "5" },
        { "g_sync.p_ac[0]",                     "2" , "5" },
        { "g_sync.p_ac[1]",                     "2" , "5" },
        { "g_sync.p_ac[2]",                     "2" , "5" },
        { "rtc.SecMinHourDay",                  "1" , "5" },
        { "rtc.DaWeMonthYear",                  "1" , "5" },
        { "g_sync.p_pv_lp",                     "1" , "5" },       
        { "g_sync.p_accu",                      "2" , "5" },
        { "bms.u_total",                        "1" , "5" },
        { "bms.SOEpercent_total",               "2" , "5" },    
        { "rtc.SecMinHourDay",                  "2" , "5" },
        { "rtc.DaWeMonthYear",                  "2" , "5" },       
        { "dd.e_inverter_inj",                  "2" , "5" },
        { "dd.e_inverter_cons",                 "2" , "5" },
        { "dd.e_grid_inj",                      "2" , "5" },
        { "dd.e_grid_cons",                     "2" , "5" },
        { "dd.e_compensation",                  "2" , "5" },
        { "dd.q_acc",                           "2" , "5" },
        { "dd.db_asc",                          "2" , "5" },
        { "dd.hour_block_ext",                  "2" , "5" },
        { "dd.day_block_ext",                   "2" , "5" },
        { "dd.month_block_ext",                 "2" , "5" },
        { "dd.year_block_ext",                  "2" , "5" },
        { "g_sync.u_l_rms[0]",                  "2" , "5" },
        { "g_sync.u_l_rms[1]",                  "2" , "5" },
        { "g_sync.u_l_rms[2]",                  "2" , "5" }, 
        { "g_sync.p_ac[0]",                     "2" , "5" },
        { "g_sync.p_ac[1]",                     "2" , "5" },
        { "g_sync.p_ac[2]",                     "2" , "5" },
        { "g_sync.q_ac",                        "2" , "5" },
        { "g_sync.u_sg_avg[0]",                 "2" , "5" },
        { "g_sync.u_sg_avg[1]",                 "2" , "5" },
        { "g_sync.p_pv_lp",                     "2" , "5" },
        { "gd[0].f_l_slow",                     "2" , "5" },
        { "iso.r",                              "2" , "5" },
        { "inv.p_ac_house_set_new",             "2" , "2" },
        { "nsm.p_lim_proz_es",                  "2" , "2" },
        { "nsm.p_lim_proz",                     "2" , "2" },
        { "fm.error_bits[0]",                   "2" , "5" },
        { "fm.error_bits[1]",                   "2" , "5" },
        { "warn_bf",                            "2" , "5" },       
        { "prime_sm.ext_status",                "2" , "5" },
        { "rs.tb_status",                       "2" , "5" },       
        { "rs.dist_board_on",                   "2" , "5" }, 
        { "bms.Status_BMS.Allbits",             "2" , "5" },
        { "prime_sm.inverter_mode",             "2" , "5" },      
        { "sw_version",                         "2" , "5" },
        { "pic_version",                        "2" , "5" },
        { "rs.db_version",                      "2" , "5" },       
        { "dev_serial_num",                     "2" , "5" },
    ...
    public ADspData(String name, int varSize, DspVarListener listner, long refreshPeriod) throws Exception{
        if(Util.userId < 1) throw new Exception("Library initialization error");
        if (varSize <= 0) throw new RuntimeException("wrong parameters");
    ...
    private void setPerission(String name, int rPerm, int wPerm) throws Exception{
        ...
        for (String[] entry : PermList) {
            if (entry[0].equals(name)) {
                if(rPerm >= Integer.parseInt(entry[1])) readEnable = true;
                if(wPerm >= Integer.parseInt(entry[2])) writeEnable = true;
            }
        }
    }
    ...
    public boolean isValid() {
        return  (System.currentTimeMillis() < 
                (refreshTime + validTimeAfterRefresh));
    }
     ...
     * (Use 'refreshTime() != 0' to check data was refreshed)
     */
     ...
     * Use if value affected indirect. To prevent unnecessary 
     * reading immediately after modification use canReadAfterModify().
     */
     ...
     * Use if some data items must be simultaneously read (single 
     * TCP/IP message) to prevent data interference by a multi 
     * client server.    
    ...
    synchronized boolean isRefreschRequired() {
        if(!readEnable) return false;
        if (refreshRequired) {
            return true;
        }
        if ((System.currentTimeMillis() > (refreshTime + refrechPeriod))
                && (refrechPeriod > 0)) {
            return true;
        } else {
            return false;
