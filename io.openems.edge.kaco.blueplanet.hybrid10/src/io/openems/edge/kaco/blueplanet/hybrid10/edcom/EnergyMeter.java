...
public final class EnergyMeter implements DataSet {
     ...
     *
     * @param timePeriod to calculate Ampere-hour counter (use static
     * definitions EnergyMeter.DAY, EnergyMeter.MONTH or EnergyMeter.YEAR)
    ...
    public String toString() {
        ...
        try {
            ...
            s = s + String.format("Inverter Feed In     \t\t%.1f kWh    \t%.1f kWh    \t%.1f kWh",
                    getEInverterFeedIn(EnergyMeter.DAY) / 1000, getEInverterFeedIn(EnergyMeter.MONTH) / 1000, getEInverterFeedIn(EnergyMeter.YEAR) / 1000);
            s = s + String.format("\nInverter Cons.     \t\t%.1f kWh    \t%.1f kWh    \t%.1f kWh",
                    getEInverterCons(EnergyMeter.DAY) / 1000, getEInverterCons(EnergyMeter.MONTH) / 1000, getEInverterCons(EnergyMeter.YEAR) / 1000);
            s = s + String.format("\nGrid Feed In       \t\t%.1f kWh    \t%.1f kWh    \t%.1f kWh",
                    getEGridFeedIn(EnergyMeter.DAY) / 1000, getEGridFeedIn(EnergyMeter.MONTH) / 1000, getEGridFeedIn(EnergyMeter.YEAR) / 1000);
            s = s + String.format("\nCons. from grid    \t\t%.1f kWh    \t%.1f kWh    \t%.1f kWh",
                    getEConsFromGrid(EnergyMeter.DAY) / 1000, getEConsFromGrid(EnergyMeter.MONTH) / 1000, getEConsFromGrid(EnergyMeter.YEAR) / 1000);
            s = s + String.format("\nSelf consumption   \t\t%.1f kWh    \t%.1f kWh    \t%.1f kWh",
                    getESelfConsumption(EnergyMeter.DAY) / 1000, getESelfConsumption(EnergyMeter.MONTH) / 1000, getESelfConsumption(EnergyMeter.YEAR) / 1000);
