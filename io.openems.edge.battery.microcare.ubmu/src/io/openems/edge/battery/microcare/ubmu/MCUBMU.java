...
/**
 * Component factory class for interfacing with the Microcare Universal Battery Management Unit (MCUBMU), currently in development
 */
...
public class MCUBMU extends AbstractMCCommsComponent implements OpenemsComponent, Battery {
	...
	/**
	 * {@link QueryTask}s used to interface with the MCUBMU via a {@link io.openems.edge.bridge.mccomms.MCCommsBridge}
	 */
	...
	/**
	 * Utility for mapping two input channels to one output channel (specifically for net current in this case)
	 */
	...
	public enum ChannelId implements io.openems.edge.common.channel.ChannelId {
		/**
		 * Signed value representing the current being used to charge the battery being managed by the MCUBMU
		 * <ul>
		 *     <li>
		 *         Type: Integer
		 *     </li>
		...
		/**
		 * Signed value representing the current flowing out of the battery being managed by the MCUBMU
		 * <ul>
		 *     <li>
		 *         Type: Integer
		 *     </li>
	...
	public MCUBMU() {
		...
		channel(ChannelId.DISCHARGE_CURRENT).onSetNextValue(value -> netCurrentChannelUpdater.dischargeCurrentChannelUpdated());
		channel(ChannelId.CHARGE_CURRENT).onSetNextValue(value -> netCurrentChannelUpdater.chargeCurrentChannelUpdated());
	}
	
	/**
	 * {@link Reference}-annotated method to set the {@link io.openems.edge.bridge.mccomms.MCCommsBridge} for this device
		...
		try {
			...
			queryTasks.add(
					constructQueryTask(
							...
							MCCommsElement.newInstanceFromChannel(13, 2, channel(ChannelId.CHARGE_CURRENT))
									.setScaleFactor(100),
	 ...
	 * @param queryCommand the command value to use when querying the MCUBMU
	 * @param responseCommand the expected command value of the reply packet
	 * @param responsePacketElements the elements used to create a packet structure for the reply packet to map to
	...
	/**
	 * Private class that offsets the {@link ChannelId#CHARGE_CURRENT} and {@link ChannelId#DISCHARGE_CURRENT} channel values against each other to
	 * produce a net current value that can be mapped to the {@link Battery.ChannelId#CURRENT} channel
	 */
	private class NetCurrentChannelUpdater {
		/**
		 * Flag that signals if the {@link ChannelId#CHARGE_CURRENT} channel has been updated since the last time a value was mapped to the {@link Battery.ChannelId#CURRENT} channel
		 */
		private boolean chargeCurrentChannelUpdated;
		/**
		 * Flag that signals if the {@link ChannelId#DISCHARGE_CURRENT} channel has been updated since the last time a value was mapped to the {@link Battery.ChannelId#CURRENT} channel
		 */
		...
		/**
		 * Set the {@link NetCurrentChannelUpdater#dischargeCurrentChannelUpdated} flag to true and calls {@link NetCurrentChannelUpdater#tryUpdateNetCurrentChannelValue()}
		 */
		...
		/**
		 * Set the {@link NetCurrentChannelUpdater#chargeCurrentChannelUpdated} flag to true and calls {@link NetCurrentChannelUpdater#tryUpdateNetCurrentChannelValue()}
		 */
		...
		/**
		 * If both the {@link NetCurrentChannelUpdater#chargeCurrentChannelUpdated} and {@link NetCurrentChannelUpdater#dischargeCurrentChannelUpdated} flags
		 * are true, it calculates the net current and maps that value to the {@link Battery.ChannelId#CURRENT} channel
		 */
