...
public class BrowserWebsocket
	...
	protected BrowserWebsocket(int port, MetadataService metadata) throws OpenemsException {
	...
	protected void _onOpen(WebSocket websocket, ClientHandshake handshake) {
		...
		if (!data.getOdooSessionId().isPresent()) {
			...
			try {
				this.metadata.getInfoWithSession(session.getData().getOdooSessionId().get());
			} catch (OpenemsException e) {
				error = e.getMessage();
			}
		}
