...
public class WriteTask extends Task {
	...
	public WriteTask(String sourceId, int modbusUnitId) {
	...
	//
	// public WriteTask(int modbusUnitId, ModbusBridge bridge, WriteableModbusRange
	// range) {
	...
	// /*
	// * Build a list of start-addresses of elements without holes. The
	// * start-addresses map to a list of elements and their values (sorted by order
	// * of insertion using LinkedHashMap)
	// */
	// LinkedHashMap<Integer, LinkedHashMap<ModbusElement<?>, Boolean>> elements =
	// new LinkedHashMap<>();
	// Integer nextStartAddress = null;
	// LinkedHashMap<ModbusElement<?>, Boolean> values = new
	// LinkedHashMap<ModbusElement<?>, Boolean>();
	...
	// value = false;
	// } else if (element.getChannel() != null && element.getChannel() instanceof
	// ModbusCoilWriteChannel) {
	// Optional<Boolean> valueOptional = ((ModbusCoilWriteChannel)
	// element.getChannel()).writeShadowCopy();
	...
	// * Write all elements
	// */
	// for (Entry<Integer, LinkedHashMap<ModbusElement<?>, Boolean>> entry :
	// elements.entrySet()) {
	// /*
	...
	// log.error("Bridge [" + modbusBridge.id() + "], Range [" +
	// range.getStartAddress() + "/0x"
	...
	// * of insertion using LinkedHashMap)
	// */
	// LinkedHashMap<Integer, LinkedHashMap<ModbusElement<?>, Long>> elements = new
	// LinkedHashMap<>();
	// Integer nextStartAddress = null;
	// LinkedHashMap<ModbusElement<?>, Long> values = new
	// LinkedHashMap<ModbusElement<?>, Long>();
	// ;
	// for (ModbusElement<?> element : range.getElements()) {
	// // Test if Channel is a dummy or writable and receive write value
	// long value = 0L;
	...
	// value = 0L;
	// } else if (element.getChannel() != null && element.getChannel() instanceof
	// ModbusWriteLongChannel) {
	// Optional<Long> valueOptional = ((ModbusWriteLongChannel)
	// element.getChannel()).writeShadowCopy();
	// if (valueOptional.isPresent()) {
	// value = valueOptional.get();
	// } else {
	...
	// * Write all elements
	// */
	// for (Entry<Integer, LinkedHashMap<ModbusElement<?>, Long>> entry :
	// elements.entrySet()) {
	// /*
	// * Get start address and all registers
	// */
