...
| `externalAuthEnabled` | Enable trusted external authentication headers | Boolean | `false`
| `externalAuthTrustedProxyCidrs` | IPv4 CIDR ranges that may provide external authentication headers | String[] | (empty)
| `externalAuthUserIdHeader` | Header containing the externally authenticated user id | String | `X-OpenEMS-User`
| `externalAuthUserNameHeader` | Header containing the externally authenticated display name | String | `X-OpenEMS-Name`
| `externalAuthRoleHeader` | Header containing the OpenEMS role | String | `X-OpenEMS-Role`
| `externalAuthDefaultRole` | Fallback role if no role header is present | String | `guest`
|===
...
Trusted external authentication is disabled by default.
If enabled, the WebSocket controller can authenticate a connection from headers that were added by a trusted reverse proxy.
This allows deployments where an identity provider or reverse proxy performs the actual login and OpenEMS Edge only consumes the resulting user attributes.

The direct peer address must match `externalAuthTrustedProxyCidrs`; otherwise all external authentication headers are ignored.
This prevents clients from authenticating themselves by sending the same headers directly to Edge.
...
.Example JSON-RPC authentication
[source,json]
----
{
  "jsonrpc": "2.0",
...
The supported roles are `admin`, `installer`, `owner` and `guest`.
Unknown roles fall back to `guest`.
Local password and token authentication remain unchanged and can be used as fallback.
