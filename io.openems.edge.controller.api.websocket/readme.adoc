...
* <<_api_rest,API REST>> - HTTP REST alternative
* <<_api_mqtt,API MQTT>> - MQTT publish/subscribe alternative
* <<_api_modbus,API Modbus>> - Industrial protocol alternative
* <<_api_backend,API Backend>> - Backend remote connection
* <<_controller_api,Controller API>> - Base controller interface

## Performance Considerations

* **Connection Limit**: OS file descriptor limits; verify with `ulimit -n`
* **Message Size**: Large configuration updates create larger messages; bandwidth may be bottleneck
* **Cycle Frequency**: Real-time updates every cycle (~500ms); high channel count increases message volume
* **Client Count**: Each client connection uses memory and thread resources; limit concurrent clients if needed

For many concurrent UIs, consider load-balancing across multiple Edge instances or using REST API for non-real-time queries.

## Troubleshooting

.Cannot connect to WebSocket
Verify port is correct, firewall allows WebSocket traffic (port 8085), service is enabled and started.

.Connection drops frequently
Check network stability, increase `apiTimeout` if network is slow, review server logs for errors.

.Slow updates or high latency
Check channel subscription list (too many channels?), verify network bandwidth, review thread pool utilization.

.Authentication fails
Check username/password are correct, user exists in OpenEMS configuration, credentials are base64-encoded properly.

.UI shows stale data
Channel may have expired due to `apiTimeout`. Verify UI is actively sending writes/subscriptions.