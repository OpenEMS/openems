    ...
    private void reactToComponentCommand(MqttCommandType key, CommandWrapper value) {
        if (super.otherComponent instanceof MqttCommands) {
            ...
            try {
                switch (key) {
                    case SETPOWER:
                        commandChannel.getSetPowerChannel().setNextWriteValue(value.getValue());
                        break;
                    case SETPERFORMANCE:
                        commandChannel.getSetPerformanceChannel().setNextWriteValue(value.getValue());
                        break;
                    case SETSCHEDULE:
                        commandChannel.getSetScheduleChannel().setNextWriteValue(value.getValue());
                        break;
                    case SETTEMPERATURE:
                        commandChannel.getSetTemperatureChannel().setNextWriteValue(value.getValue());
                        break;
                }
