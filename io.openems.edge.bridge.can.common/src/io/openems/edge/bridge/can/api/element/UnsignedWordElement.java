...
/**
 * An UnsignedWordElement represents an Integer value (16 bit) in an
 * {@link AbstractWordElement}.
 */
public class UnsignedWordElement extends AbstractCanChannelElement<UnsignedWordElement, Integer> {
	 ...
	 * @param valueoffset value to add to the received unsigned word
	 * @param valuemin    min possible value
	 * @param valuemax    max possible value
	 */
	public UnsignedWordElement(int bitposition, int bitcnt, int valueoffset, int valuemin, int valuemax)
	...
	public UnsignedWordElement(int bitposition, int bitcnt, int valueoffset, int valuemin, int valuemax,
		...
		if (bitposition % 8 != 0 || bitcnt > 16) {
			throw new OpenemsException(
					"Invalid 'position' or 'len' within definition of UnsignedWordElement. bitposition: " + bitposition
							+ " bitcnt: " + bitcnt);
		}
	}

	@Override
	public UnsignedWordElement self() {
		return this;
	}
	...
	public void doElementSetInput(byte[] data) throws CanIoException {
		...
		var buff = ByteBuffer.allocate(2).order(this.getByteOrder());
		buff.put(valbytes);
		var value = this.fromByteBuffer16(buff);
