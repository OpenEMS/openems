...
public class PlcNextGdsDataProviderImpl implements PlcNextGdsDataProvider {
	...
	private TimeEndpoint<HttpResponse<String>> maintainSessionTimeEndpoint;
	...
	public synchronized CompletableFuture<JsonObject> readDataFromRestApi(List<String> variableIdentifiers,
		...
		return this.ensureAccessTokenAndSessionIdAreValid(dataAccessConfig, authConfig) //
				.thenCompose(ignore -> {
					var requestBody = this.buildPostBodyForRead(this.sessionId, variableIdentifiers, dataAccessConfig);
					var dataEndPoint = this.buildDataEndpointRepresentation(this.tokenManager.getToken(), //
							HttpMethod.POST, requestBody, dataAccessConfig);

					return this.sendRequestToApi(dataEndPoint, HttpStatus.OK, dataAccessConfig.stationId());
				});
	}
	...
	public synchronized CompletableFuture<JsonObject> writeDataToRestApi(List<JsonElement> mappedVariables,
		...
		if (Objects.isNull(mappedVariables)) {
			...
			return this.ensureAccessTokenAndSessionIdAreValid(dataAccessConfig, authConfig) //
					.thenCompose(ignore -> {
						var requestBody = this.buildPutBodyForWrite(this.sessionId, mappedVariables);
						var dataEndPoint = this.buildDataEndpointRepresentation(this.tokenManager.getToken(),
								HttpMethod.PUT, requestBody, dataAccessConfig);
	...
	public synchronized void deactivateSessionMaintenance(PlcNextGdsDataAccessConfig config) {
		this.deactivateSessionMaintenanceIfNecessary(config);
	}
	...
	private void deactivateSessionMaintenanceIfNecessary(PlcNextGdsDataAccessConfig config) {
		...
		if (Objects.nonNull(this.maintainSessionTimeEndpoint)) {
			log.info("StationID '{}': Deactivating session maintenance for sessionID {}", config.stationId(),
					oldSessionId);
			this.timeService.removeTimeEndpoint(this.maintainSessionTimeEndpoint);
		}
		this.maintainSessionTimeEndpoint = null;
	}

	/**
	 * Checks access token and session and triggers fetching both if not existing or
	 * invalid.
	 *
	 ...
	 */
	CompletableFuture<Void> ensureAccessTokenAndSessionIdAreValid(PlcNextGdsDataAccessConfig dataAccessConfig, //
		...
		if (!this.tokenManager.hasValidToken()) {
			...
			result = this.tokenManager.fetchToken(authConfig);
		}
		if (this.canCreateSession(dataAccessConfig)) {
			result = result.thenCompose(ignore -> this.createOrFetchSessionID(dataAccessConfig))
					.thenApply(createSessionResponse -> {
						if (!createSessionResponse.sessionId.isBlank()) {
							this.sessionId = createSessionResponse.sessionId();

							if (this.canActivateSessionMaintenance()) {
								this.maintainSessionTimeEndpoint = this.enableSessionMaintenance(
	 ...
	 * maintenance while access token is valid and no error occurs when there is no
	 * active session. It sets the member variables "sessionId" and
	 * "maintainSessionEndpoint".
	 *
	...
	CompletableFuture<PlcNextCreateSessionResponse> createOrFetchSessionID(PlcNextGdsDataAccessConfig config) {
		...
		try {
			...
			return this.fetchSession(config);
		}
	}
	...
	private CompletableFuture<PlcNextCreateSessionResponse> createSession(PlcNextGdsDataAccessConfig config) {
		// create session
		var createSessionEndpoint = this.buildCreateSessionEndpoint(this.tokenManager.getToken(), config);
		log.info("StationID '{}': Create session using endpoint URL: {}", config.stationId(),
				createSessionEndpoint.url());

		return this.sendRequestToApi(createSessionEndpoint, HttpStatus.CREATED, config.stationId()) //
				.thenApply(createSessionBody -> {
					log.debug("StationID '{}': Create session body: {}", config.stationId(), createSessionBody);

					if (Objects.isNull(createSessionBody)) {
						return null;
					} else {
						var newSessionId = createSessionBody.get("sessionID").getAsString();
						var timeoutDuration = Duration.ofMillis(createSessionBody.get("timeout").getAsLong())
								.minusSeconds(1L);

						return new PlcNextCreateSessionResponse(newSessionId, timeoutDuration);
					}
				});
	}
	...
	private CompletableFuture<PlcNextCreateSessionResponse> fetchSession(PlcNextGdsDataAccessConfig config) {
		// fetch session
		Endpoint fetchSessionsEndpoint = this.buildFetchSessionsEndpoint(this.tokenManager.getToken(), config);
		log.info("StationID '{}': Fetch sessions using endpoint URL: {}", config.stationId(),
				fetchSessionsEndpoint.url());

		return this.sendRequestToApi(fetchSessionsEndpoint, HttpStatus.OK, config.stationId()) //
				.thenApply(fetchSessionsResponseBody -> {
					log.debug("StationID '{}': Fetch sessions response body: {}", config.stationId(),
							fetchSessionsResponseBody);
						...
						if (Objects.nonNull(sessions) && !sessions.isJsonNull() && sessions.isJsonArray()) {
							Optional<JsonObject> sessionJsonObject = sessions.getAsJsonArray().asList().stream() //
									.filter(item -> item.isJsonObject() //
											...
											&& config.stationId().equalsIgnoreCase(
													item.getAsJsonObject().get("stationID").getAsString()))
							...
							if (sessionJsonObject.isEmpty()) {
								...
								Duration timeoutDuration = Duration
										.ofMillis(fetchSessionsResponseBody.get("timeout").getAsLong())
										.minusSeconds(1L);

								result = new PlcNextCreateSessionResponse(sessionId, timeoutDuration);
								log.debug("StationID '{}': Session of this station found {}.", config.stationId(),
										result);
							}
						}
					}
	 ...
	 * @param config         config of base URL and instance name
	 * @return @link{TimeEndpoint} representing the continuous session maintenance
	 *         or NULL otherwise
	 */
	TimeEndpoint<HttpResponse<String>> enableSessionMaintenance(Delay sessionTimeout,
			PlcNextGdsDataAccessConfig config) {

		// trigger session maintenance
		var delayTimeProvider = new DefaultDelayTimeProvider<HttpResponse<String>>(//
				() -> sessionTimeout, //
				error -> Delay.infinite(), //
				result -> sessionTimeout);
		var maintainSessionEndpoint = this.buildMaintainSessionEndpoint(//
				this.tokenManager.getToken(), this.sessionId, config);
		log.info("SessionID '{}': Maintaining session using endpoint URL: {}", this.sessionId,
				maintainSessionEndpoint.url());

		return this.timeService.subscribeJsonTime(delayTimeProvider, maintainSessionEndpoint,
				(httpResponse, httpError) -> {
					if (Objects.isNull(httpResponse) && Objects.isNull(httpError)) {
						...
						log.error("SessionID '{}': Got HTTP error '{}'! Session ID will be reset.", //
								this.sessionId, httpError.getMessage());
						this.deactivateSessionMaintenanceIfNecessary(config);
					} else if (!this.tokenManager.hasValidToken()) {
						// Stop on expired token
						log.info(
								"SessionID '{}': Got result, but access token has been expired. Session ID will be reset.",
								this.sessionId);
						this.deactivateSessionMaintenanceIfNecessary(config);
					} else if (httpResponse.status() == HttpStatus.OK && Objects.nonNull(httpResponse.data())
	 ...
	 *
	 * @param endpoint       represents the endpoint definition to be used for the*
	 *                       request
	 * @param expectedStatus represents the expected status to state a successful*
	 *                       API request
	...
	CompletableFuture<JsonObject> sendRequestToApi(Endpoint endpoint, HttpStatus expectedStatus, //
		...
		try {
			log.debug("StationID '{}': Sending request to API endpoint URL: '{}'", stationId, endpoint.url());
			CompletableFuture<HttpResponse<JsonElement>> requestFuture = this.http.requestJson(endpoint); //

			if (Objects.isNull(requestFuture)) {
				log.error("Cannot process response due to NULL!");
				return CompletableFuture.failedFuture(new NullPointerException());
			}
			return requestFuture.thenApply(apiResponse -> {
				if (expectedStatus == apiResponse.status()) {
					...
					throw new IllegalStateException(new StringBuilder() //
							...
							.append("and body: '").append(apiResponse.data()).append("'") //
							.toString());
				}
			});
		} catch (CompletionException e) {
			log.error("StationID '{}': Error while sending API request! Request body: {}", //
					stationId, endpoint.body(), e);
	 ...
	 */
	Endpoint buildCreateSessionEndpoint(String authToken, PlcNextGdsDataAccessConfig config) {
		var createSessionEndpointUrl = PlcNextUrlStringHelper.buildUrlString(config.dataUrl(), PATH_SESSIONS);
		var headers = Map.of("Accept", "application/json", //
				"Content-Type", "text/plain", //
				"Authorization", "Bearer " + authToken);
		var postRequestBody = new StringBuilder("stationID=") //
	 ...
	 * @return @link{Endpoint} object
	 */
	Endpoint buildFetchSessionsEndpoint(String authToken, PlcNextGdsDataAccessConfig config) {
		var fetchSessionEndpointUrl = PlcNextUrlStringHelper.buildUrlString(config.dataUrl(), PATH_SESSIONS);
		var headers = Map.of("Accept", "application/json", //
				"Authorization", "Bearer " + authToken);
	 ...
	 *
	 * @param authToken the auth token of PLCnext REST-API
	 * @param sessionId Id of current PLCnext session
	 * @param config    config of base URL and instance name
	 * @return @link{Endpoint} object
	 */
	Endpoint buildMaintainSessionEndpoint(String authToken, String sessionId, PlcNextGdsDataAccessConfig config) {
		var maintainSessionEndpointUrl = new StringBuilder(
				PlcNextUrlStringHelper.buildUrlString(config.dataUrl(), PATH_SESSIONS))//
				.append("/").append(sessionId).toString();
		var headers = Map.of("Accept", "application/json", "Authorization", "Bearer " + authToken);
	 ...
	 * @return @link{Endpoint} object
	 */
	Endpoint buildDataEndpointRepresentation(String authToken, HttpMethod method, String requestBody,
			PlcNextGdsDataAccessConfig config) {
		var dataEndpointUrl = PlcNextUrlStringHelper.buildUrlString(config.dataUrl(), PATH_VARIABLES);
		var headers = Map.of("Accept", "application/json", //
	 ...
	 * @return request body
	 */
	String buildPostBodyForRead(String sessionId, List<String> variableIdentifiers, PlcNextGdsDataAccessConfig config) {
		var postRequestBody = "";

		if (Objects.nonNull(variableIdentifiers) && !variableIdentifiers.isEmpty()) {
			var variableNames = variableIdentifiers.stream()//
	 ...
	 * @return request body
	 */
	String buildPutBodyForWrite(String sessionId, List<JsonElement> mappedVariables) {
	...
	/**
	 * Protected access to session ID for session keep alive testing.
	 *
