...
public class PlcNextGdsDataProviderImpl implements PlcNextGdsDataProvider {
     ...
     * @return @link{CompletableFuture} containing an object of type @link{PlcNextCreateSessionResponse}
    ...
    TimeEndpoint<HttpResponse<String>> enableSessionMaintenance(Delay sessionTimeout,
        ...
        var delayTimeProvider = new DefaultDelayTimeProvider<HttpResponse<String>>(() -> sessionTimeout, error -> Delay.infinite(), result -> sessionTimeout);
        var maintainSessionEndpoint = this.buildMaintainSessionEndpoint(this.tokenManager.getToken(), this.sessionId, config);
        log.info("SessionID '{}': Maintaining session using endpoint URL: {}", this.sessionId, maintainSessionEndpoint.url());
                ...
                maintainSessionEndpoint, (httpResponse, httpError) -> {
                    if (Objects.isNull(httpResponse) && Objects.isNull(httpError)) {
                        ...
                        log.info("SessionID '{}': Got result, but access token has been expired. Session ID will be reset.",
                                this.sessionId);
                        this.deactivateSessionMaintenanceIfNecessary(config);
                    } else if (httpResponse.status() == HttpStatus.OK && Objects.nonNull(httpResponse.data()) && Objects.nonNull(httpResponse.data().getAsJsonObject()) && Objects.nonNull(httpResponse.data().getAsJsonObject().get("sessionID"))) {
                        // Success
     ...
     * @return @link{Endpoint} object
     */
    Endpoint buildMaintainSessionEndpoint(String authToken, String sessionId,
                                          PlcNextGdsDataAccessConfig config) {
     ...
     * @return request body
     */
    String buildPostBodyForRead(String sessionId, List<String> variableIdentifiers,
                                PlcNextGdsDataAccessConfig config) {
        var postRequestBody = "";
