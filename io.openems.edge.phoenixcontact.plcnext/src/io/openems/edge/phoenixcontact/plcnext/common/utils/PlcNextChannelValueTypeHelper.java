...
public final class PlcNextChannelValueTypeHelper {
	...
	private static Object mapToEnum(JsonElement jsonElement, Doc openEmsChannelDoc, String stationId) {
		...
		if (mappedValues.isEmpty()) {
			log.warn("StationID '{}': Channel options {} do not contain option {}, using UNDEFINED value.", stationId,
					Arrays.toString(options), sourceValue);
			mappedValue = ((EnumDoc) openEmsChannelDoc).getUndefinedOption();
		} else if (mappedValues.size() > 1) {
			log.warn("StationID '{}': Multiple options found for source value '{}', choosing first.", stationId,
					sourceValue);
	...
	/**
	 * Helper method to build single JSON object that contains a path to value
	 * assignment to be written to REST-API.
	 * 
	...
	public static JsonElement buildVariableToWrite(String variablePath, Object variableValue, Doc openEmsChannelDoc,
		...
		if (Objects.isNull(variableValue)) {
			log.debug("StationID '{}': Channel value is NULL! Skipping processing of variable '{}'.", stationId,
					variablePath);
		} else {
			...
			if (isOpenEmsEnumType(openEmsChannelDoc)) {
				mappedValue.addProperty(PlcNextChannelToGdsDataMapper.PLC_NEXT_VARIABLE_VALUE,
						((OptionsEnum) variableValue).getName());
			} else if (isOpenEmsTypeFloat(openEmsChannelDoc)) {
