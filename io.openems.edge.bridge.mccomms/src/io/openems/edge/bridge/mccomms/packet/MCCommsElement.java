...
public class MCCommsElement {
	 ...
	 * @param isUnsigned Whether to treat values as unsigned during buffer operations
	...
	/**
	 * Static constructor for an unscaled numerical element with no bound {@link Channel}; usually a command or address field
	 * @param startAddress the index of the first byte of this element within a {@link MCCommsPacket} buffer
	 * @param numBytes the number of bytes within a {@link MCCommsPacket} buffer this element uses
	 * @param value the number value to assign to the new element
	 * @param isUnsigned Whether to treat values as unsigned during buffer operations
	 * @return a new MCCommsElement with a 1.0 scaling factor and no bound {@link Channel}
	 * @throws OpenemsException if an abnormal buffer length is specified by {@code numBytes}
	 */
	...
	/**
	 * Static constructor to create an MCCommsElement bound to a specified {@link Channel}
	 * @param startAddress the index of the first byte of this element within a {@link MCCommsPacket} buffer
	 * @param numBytes the number of bytes within a {@link MCCommsPacket} buffer this element uses
	 * @param channel {@link Channel} to bind to this element
	 ...
	 * @throws OpenemsException if the internal {@link ByteBuffer} has a capacity not equal the number of supplied bytes
	 */
	...
	/**
	 * @return amount by which to scale values when pulling and pushing values to the buffer
	 */
	...
	/**
	 * Sets the amount by which to scale values when pulling and pushing values to the buffer
	...
	/**
	 * Reads the element value from the internal {@link ByteBuffer}, scales it, and pushes it to the bound {@link Channel}
	 * @throws OpenemsException if the {@link io.openems.common.types.OpenemsType} of the bound {@link Channel}, or there
	 * is no channel bound to the current instance
	 */
	...
	/**
	 * Gets a value from the internal {@link ByteBuffer} and applies the scaling factor
	 * @see MCCommsElement#scaleFactor
	 * @return the scaled numerical value of this element instance
	 * @throws OpenemsException if the buffer has an abnormal length or an attempt is made to treat a 64-bit value as unsigned
	 */
	...
	/**
	 * Pulls a {@link io.openems.edge.common.channel.value.Value} from the bound {@link Channel}, scales it if applicable, and pushes it to the internal {@link ByteBuffer}
	 * @throws OpenemsException if the buffer has an abnormal length, the {@link Channel} {@link io.openems.common.types.OpenemsType} is not supported,
	 * or there is no {@link Channel} bound to the current element
	 */
