...
public class ListenTask implements Future<MCCommsPacket> {
	...
	/**
	 * The MCComms address to which incoming packets must be addressed to in order to be mapped by this task
	 */
	private int destinationAddress;
	/**
	 * The mandatory value of the command field of the incoming packet in order for the this task to map the incoming buffer
	 */
	private int command;
	/**
	...
	/**
	 * Any other conditions that the incoming buffer must pass in the form of {@link ByteBuffer} consuming {@link Predicate}s
	 */
	private Predicate<ByteBuffer>[] otherConditions;
	/**
	 * Atomic boolean used to ensure interthread synchronisation for this {@link Future}-implementing class
	 */
	private AtomicBoolean hasReturned;
	
	/**
	 * Constructor
	 * @param sourceAddress the MCComms address from which incoming packets must originate in order to be mapped by this task
	 * @param destinationAddress the MCComms address to which incoming packets must be addressed to in order to be mapped by this task
	 * @param command the mandatory value of the command field of the incoming packet in order for the this task to map the incoming buffer
	 * @param packet the defined packet structure to which the incoming buffer must be mapped
	 * @param otherConditions any other conditions that the incoming buffer must pass in the form of {@link ByteBuffer} consuming {@link Predicate}s
	 */
	@SafeVarargs
	...
	/**
	 * Internal method to ensure all supplied additional conditions (see {@link ListenTask#ListenTask(int, int, int, MCCommsPacket, Predicate[])}) return true
	...
	/**
	 * Consumes a buffer, and if the source address, destination address, command and all other supplied conditions
	 * ({@link ListenTask#otherConditions}) match or return true, maps the buffer to a defined packet structure
	 * @param buffer the buffer to consume
	 * @throws OpenemsException if the buffer cannot be mapped to the packet structure
	 */
