...
/**
 * Class to represent packets that must be written out to the serial bus via an
 * {@link MCCommsBridge} Capable of repeatedly writing out
 */
public class WriteTask {
	...
	/**
	 * Static constructor for sending a packet with an empty payload and custom
	 * command value
	 * 
	 * @param sourceAddress      The MCComms address to which the packet must be
	 *                           sent
	 * @param destinationAddress The MCComms address from which this packet must
	 *                           appear to originate
	 ...
	 */
	static WriteTask newCommandOnlyWriteTask(int sourceAddress, int destinationAddress, int command)
			throws OpenemsException {
		return new WriteTask(new MCCommsPacket(MCCommsElement.newUnscaledNumberInstance(1, 2, destinationAddress, true),
	...
	/**
	 * Method for this instance to repeatedly schedule itself to be written out by a
	 * {@link MCCommsBridge} instance
	 * 
	...
	public ScheduledFuture<?> sendRepeatedly(IMCCommsBridge bridge, long timePeriod, TimeUnit timeUnit) {
		return bridge.getScheduledExecutorService().scheduleAtFixedRate(() -> bridge.addWriteTask(this), 0, timePeriod,
				timeUnit);
	}
