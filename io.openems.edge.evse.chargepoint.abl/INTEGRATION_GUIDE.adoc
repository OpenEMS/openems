...
. *Unit Tests* - Fast, isolated tests using dummy/mock components
. *Component Tests* - OSGi lifecycle testing using `ComponentTest` framework
. *Integration Tests* - Full system tests with real dependencies
. *Manual Testing* - Simulator components for development and debugging
...
| Base Class
| AbstractDummyEvseChargePoint
| AbstractOpenemsComponent

| Builder Pattern
| Uses `self()` for fluent API
| Direct method calls
...
. `readme.adoc` - Component documentation (AsciiDoc format)
. JavaDoc on all public methods
. Test documentation in test class comments

=== ABL Documentation (Enhanced)

. ✅ `readme.md` - Component documentation (Markdown)
. ✅ `TEST_README.md` - Comprehensive testing guide
. ✅ `INTEGRATION_GUIDE.md` - This file (integration with OpenEMS)
...
. *Add Missing Standard Tests*
  ** Create `MyConfig` helper
  ** Add `ComponentTest` for real component
  ** Add controller integration tests

. *Consider OSGi Simulator Component* (Optional)
