...
public class SetPvLimitHandler implements ThrowingRunnable<OpenemsNamedException> {
	...
	private final DeyeDcChargerImpl parent;
	...
	public SetPvLimitHandler(DeyeDcChargerImpl parent, ManagedSymmetricPvInverter.ChannelId activePowerLimit) {
		this.parent = parent;
	...
	public void run() throws OpenemsNamedException {
		...
		if (!Objects.equals(this.lastPLimitPerc, pLimitPerc) || this.lastPLimitPercTime
			...
			IntegerWriteChannel pLimitPercCh = this.parent.channel(DeyeDcCharger.ChannelId.P_LIMIT_PERC);
			pLimitPercCh.setNextWriteValue(pLimitPerc);

			EnumWriteChannel pLimitTypeCh = this.parent.channel(DeyeDcCharger.ChannelId.P_LIMIT_TYPE);
			pLimitTypeCh.setNextWriteValue(PLimitType.FIXED_LIMIT);

			IntegerWriteChannel watchDogTagCh = this.parent.channel(DeyeDcCharger.ChannelId.WATCH_DOG_TAG);
