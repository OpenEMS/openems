...
public interface SungrowStringInverter extends OpenemsComponent {
	...
	public enum ChannelId implements io.openems.edge.common.channel.ChannelId {
		...
		DEVICE_CODE(Doc.of(OpenemsType.STRING).accessMode(AccessMode.READ_ONLY)
				.persistencePriority(PersistencePriority.VERY_LOW)),
		OUTPUT_TYPE(Doc.of(OpenemsType.STRING).accessMode(AccessMode.READ_ONLY)
				.persistencePriority(PersistencePriority.VERY_LOW)),
		NOMINAL_OUTPUT_POWER(Doc.of(OpenemsType.INTEGER).unit(Unit.WATT).accessMode(AccessMode.READ_ONLY)
				.persistencePriority(PersistencePriority.VERY_LOW)),
		DAILY_ENERGY(Doc.of(OpenemsType.INTEGER).unit(Unit.WATT_HOURS).accessMode(AccessMode.READ_ONLY)
				.persistencePriority(PersistencePriority.MEDIUM)),
		TOTAL_RUNNING_TIME(Doc.of(OpenemsType.INTEGER).unit(Unit.HOUR).accessMode(AccessMode.READ_ONLY)
				.persistencePriority(PersistencePriority.VERY_LOW)),
		INTERNAL_TEMPERATURE(Doc.of(OpenemsType.INTEGER).unit(Unit.DEGREE_CELSIUS).accessMode(AccessMode.READ_ONLY)
				.persistencePriority(PersistencePriority.VERY_LOW)),
		...
		POWER_LIMITATION_SWITCH(Doc.of(OpenemsType.BOOLEAN).unit(Unit.ON_OFF).accessMode(AccessMode.READ_WRITE)),
		POWER_LIMITATION_SETTING(Doc.of(OpenemsType.INTEGER).unit(Unit.PERCENT).accessMode(AccessMode.READ_WRITE));
