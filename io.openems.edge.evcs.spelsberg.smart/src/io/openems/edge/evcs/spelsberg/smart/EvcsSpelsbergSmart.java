...
public interface EvcsSpelsbergSmart extends OpenemsComponent {
	...
	public enum ChannelId implements io.openems.edge.common.channel.ChannelId {
		...
		EVSE_STATE(Doc.of(EvseState.values()).accessMode(AccessMode.READ_ONLY).text("State of the charging station")),
		...
		CHARGE_DURATION_SESSION(
				Doc.of(OpenemsType.INTEGER).unit(Unit.SECONDS).text("Duration of the current session in Wh")),
	...
	public default void setApplyChargePowerLimit(Integer value) throws OpenemsNamedException {
		getApplyChargePowerLimitChannel().setNextWriteValue(value);
	}
	...
	public default Integer getLifeBit() {
		Channel<Integer> channel = getLifeBitChannel();
		return channel.value().orElse(channel.getNextValue().orElse(-1));
	}
	...
	public default void setLifeBit(Integer value) throws OpenemsNamedException {
		IntegerWriteChannel channel = (IntegerWriteChannel) getLifeBitChannel();
