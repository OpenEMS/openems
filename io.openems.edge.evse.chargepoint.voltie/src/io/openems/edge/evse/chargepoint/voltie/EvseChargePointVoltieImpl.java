...
public class EvseChargePointVoltieImpl extends AbstractOpenemsModbusComponent implements EvseChargePointVoltie,
	...
	// The device has no per-phase energy counters, so the energy per phase is
	// integrated in software from the per-phase power
	...
	private Instant lastChargingEnableError = Instant.MIN;
	...
	private synchronized void extendModbusProtocol() {
		...
		protocol.addTasks(//
				...
				new FC3ReadRegistersTask(0x2000, Priority.LOW, //
						...
						// Lifetime energy in [Wh]; includes the running session from firmware
						// build 370, so it is the meter reading
						m(ElectricityMeter.ChannelId.ACTIVE_PRODUCTION_ENERGY, new UnsignedDoublewordElement(0x2016)), //
	...
	protected void onChargingEnabledWriteExecuted(ExecuteState state) {
		if (state instanceof ExecuteState.Error) {
			...
			this.lastChargingEnableError = this.now();
		}
		this.onControlWriteExecuted(state);
	}

	/**
	 * Called after execution of the FC6 writes to registers 0x000C and 0x0014. The
	 * bridge reports every failed write as {@link ExecuteState.Error}, a Modbus
	 * exception answered by the charger as well as a transport failure.
	 *
	 * @param state the {@link ExecuteState}
	 */
	protected void onControlWriteExecuted(ExecuteState state) {
		...
		if (isRejectedByDevice(state)) {
			this.logWarn(this.log, "Control write was rejected by the charge-point; "
					+ "check that Modbus control is enabled on the charger");
		} else {
			this.logWarn(this.log, "Control write failed: " + error.exception().getMessage());
		}
	}

	/**
	 * Evaluates whether a write was rejected by the charger itself.
	 *
	 * <p>
	 * The charger answers a write it does not accept with Modbus exception 0x03
	 * (illegal data value). Every other failure is transient and must not be
	 * treated as a rejection: a transport failure, a timeout, or the gateway
	 * exceptions 0x0A (control unit not ready) and 0x0B (no response from the
	 * control unit).
	 *
	 * @param state the {@link ExecuteState}
	 * @return true if the charger answered exception 0x03
	 */
	static boolean isRejectedByDevice(ExecuteState state) {
		return state instanceof ExecuteState.Error(var exception) //
				&& exception instanceof ModbusSlaveException mse //
				&& mse.isType(Modbus.ILLEGAL_VALUE_EXCEPTION);
	}
	...
	private void onBeforeProcessImage() {
		// Integrate the energy per phase from the per-phase power values
		...
		// Unplugging the vehicle ends the situation in which a phase switch was
		// rejected, so the next session may try again. Only a reported state A
		// counts: an undefined state after a communication failure does not.
		if (evseState.isEvConnected()) {
			this.evConnected = true;
		} else if (evseState == EvseState.STATE_A && this.evConnected) {
	...
	private PhaseSwitchDirection getPhaseSwitchAbility() {
		if (!this.isPhaseSwitchingSupported() // not supported by this hardware
				...
				|| this.getFirmwareBuild().orElse(0) < MIN_FIRMWARE) { // requires the minimum firmware
			return null;
		}
	...
	private void applySetPoint(int currentInAmpere) {
		try {
			...
			// Re-enable charging if it was disabled. The firmware discards the enable
			// while no vehicle is connected, so it is only written with a vehicle
			// present, see writeChargingEnabled(boolean)
			if (!this.getChargingEnabled().orElse(false) && this.getEvseState().isEvConnected()) {
	...
	/**
	 * Clears the write-once latch when the charger is in a new situation rather
	 * than ignoring a write.
	 *
	 * <p>
	 * The next {@link #applySetPoint(int)} then writes the register again. This is
	 * the case when:
	 *
	 * <ul>
	...
	void evaluateChargingEnableLatch() {
		...
		if (intended && !readBack && !this.getEvseState().isEvConnected()) {
			this.lastWrittenEnable = null;
			this.chargingEnableConfirmed = false;
			return;
		}
	 ...
	 *         been written or a rejected write is waiting for its retry
	...
	boolean writeChargingEnabled(boolean enable) throws OpenemsNamedException {
		...
		// A charger with control-by-Modbus disabled rejects every write; retrying
		// each Cycle would add a transaction to every Cycle
		if (Duration.between(this.lastChargingEnableError, this.now()).compareTo(WRITE_INTERVAL) < 0) {
			return false;
		}
	...
	private Instant now() {
		return Instant.now(this.componentManager.getClock());
	}
