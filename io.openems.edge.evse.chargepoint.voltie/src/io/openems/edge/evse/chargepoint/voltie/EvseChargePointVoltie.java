 ...
 *
 * <p>
 * Register map source: Voltie Modbus API documentation v1.3 (2026-08-26),
 * firmware V357.
 */
public interface EvseChargePointVoltie extends OpenemsComponent {
	...
	public enum ChannelId implements io.openems.edge.common.channel.ChannelId {
		...
		/** EVSE firmware build number, e.g. 357 (register 0x0001). */
		...
		AVAILABLE_CURRENT_CAPACITY(Doc.of(INTEGER)//
				...
				.persistencePriority(PersistencePriority.LOW)), //
		/**
		 * Lifetime energy in [Wh] (register 0x2016).
		 *
		 * <p>
		 * The charger updates this counter only at session end (unplug). The live
		 * cumulative meter value is provided on the ActiveProductionEnergy channel,
		 * which is integrated in software from the active power.
		 */
		LIFETIME_ENERGY(Doc.of(LONG)//
				.unit(WATT_HOURS) //
				.persistencePriority(PersistencePriority.LOW)), //
		...
		/** Firmware build is older than 357. */
		FIRMWARE_OUTDATED(Doc.of(Level.WARNING)//
				.text("Voltie firmware build is older than 357; "
						+ "phase switching and parts of the register map are unavailable")), //
