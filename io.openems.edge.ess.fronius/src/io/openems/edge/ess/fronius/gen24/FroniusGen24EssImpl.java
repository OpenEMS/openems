...
import io.openems.edge.bridge.modbus.sunspec.DefaultSunSpecModel.S124_StorCtl_Mod;
...
@Component(//
	name = "Ess.Fronius.GEN24", immediate = true, //
...
public class FroniusGen24EssImpl extends AbstractOpenemsSunSpecComponent
	...
	if (super.activate(context, config.id(), config.alias(), config.enabled(), config.modbusUnitId(), this.cm,
		"Modbus", config.modbus_id(), READ_FROM_MODBUS_BLOCK, this.config.readOnly())) {
	    return;
	}
	...
	this.mapFirstPointToChannel(FroniusGen24ChannelId.CHARGE_POWER, ElementToChannelConverter.DIRECT_1_TO_1,
		DefaultSunSpecModel.S160.MODULE_3_D_C_W);
	this.mapFirstPointToChannel(FroniusGen24ChannelId.DISCHARGE_POWER, ElementToChannelConverter.DIRECT_1_TO_1,
		DefaultSunSpecModel.S160.MODULE_4_D_C_W);
	...
	this.channel(FroniusGen24ChannelId.REQUESTED_ACTIVE_POWER).setNextValue(activePower);

	EnumWriteChannel storeControlChannel = (EnumWriteChannel) this
		.getSunSpecChannelOrError(DefaultSunSpecModel.S124.STOR_CTL_MOD);
	storeControlChannel.setNextWriteValue(S124_StorCtl_Mod.ALLOW_BOTH);
	...
	return this.channel(FroniusGen24ChannelId.CHARGE_POWER);
    }

    public Channel<Integer> getDischargePowerChannel() {
	return this.channel(FroniusGen24ChannelId.DISCHARGE_POWER);
    }
