...
The easiest way to provide all required parameters is via a class that implements the `BatteryProtectionDefintion` interface. See that interface for details on the parameters.
...
----
+
The easiest way is via a class that implements the `BatteryProtectionDefintion` interface. This will guide you through all the required battery protection parameters. Alternatively there is also an initialization that follows the Builder pattern. See the JUnit tests for examples.
