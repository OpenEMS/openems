...
public class SocomecMeter extends AbstractOpenemsApp<Property> implements OpenemsApp {
	...
	protected ThrowingBiFunction<ConfigurationTarget, EnumMap<Property, JsonElement>, AppConfiguration, OpenemsNamedException> appConfigurationFactory() {
		return (t, p) -> {
			...
			var modbus1 = "modbus1";
			var meter1 = this.getId(t, p, Property.METER_ID, "meter1");
			...
			components.add(new EdgeConfig.Component(meter1, alias, "Meter.Socomec.Threephase", //
	...
	private JsonArray buildMeterOptions() {
	...
	public AppAssistant getAppAssistant() {
		return AppAssistant.create(this.getName()) //
				.fields(JsonUtils.buildJsonArray() //
						.add(JsonFormlyUtil.buildSelect(Property.TYPE, this.buildMeterOptions())) //
						.build())
				.build();
	}
