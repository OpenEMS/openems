...
import io.openems.edge.app.FeneconHome.Property;
...
public class FeneconHome extends AbstractOpenemsApp<Property> implements OpenemsApp {
	...
	public OpenemsAppCategory getCategory() {
		return OpenemsAppCategory.INTEGRATED_SYSTEM;
	}
	...
	@Override
	protected ThrowingFunction<EnumMap<Property, JsonElement>, AppConfiguration, OpenemsNamedException> appConfigurationFactory() {
		return (p) -> {
			var essId = EnumUtils.getAsOptionalString(p, Property.ESS_ID).orElse("ess0");
			var modbusIdInternal = EnumUtils.getAsOptionalString(p, Property.MODBUS_ID_EXTERNAL).orElse("modbus0");
			var modbusIdExternal = EnumUtils.getAsOptionalString(p, Property.MODBUS_ID_INTERNAL).orElse("modbus1");
			...
			if (EnumUtils.getAsOptionalBoolean(p, Property.HAS_DC_PV2).orElse(false)) {
				var alias = EnumUtils.getAsOptionalString(p, Property.DC_PV1_ALIAS).orElse("DC-PV 2");
			...
			if (hasEmergencyReserve) {
				...
				components.add(new EdgeConfig.Component("ctrlEmergencyCapacityReserve0",
						...
						JsonUtils.buildJsonObject() //
								.addProperty("enabled", true) //
								.addProperty("ess.id", essId) //
								.addProperty("isReserveSocEnabled", emergencyReserveEnabled) //
								.addProperty("reserveSoc", emergencyReserveSoc) //
								.build()));
			}
