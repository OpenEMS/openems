	...
	public static Stream<? extends AbstractReadChannel<?>> initializeChannels(EssKacoBlueplanetGridsave50 ess) {
		...
		return Stream.of( //
				Arrays.stream(OpenemsComponent.ChannelId.values()).map(channelId -> {
					...
					case GRID_MODE:
						return new EnumReadChannel(ess, channelId, GridMode.ON_GRID);
...
//					case DIS_CUTOFF_A:
					...
					case REQUESTED_STATE:
						return new EnumWriteChannel(ess, channelId);
					case A_SF:
					case V_SF:
					case W_MAX:
					case W_MAX_SF:
					...
					case DC_VOLTAGE_SF:
						return new IntegerReadChannel(ess, channelId);
					case CURRENT_STATE:
						return new EnumReadChannel(ess, channelId, CurrentState.UNDEFINED);
					case VENDOR_OPERATING_STATE:
						return new EnumReadChannel(ess, channelId, ErrorCode.UNDEFINED);
					case AC_ENERGY:
						return new LongReadChannel(ess, channelId);
					}
					return null;
