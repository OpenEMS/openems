...
public class SurplusFeedInHandler {
	...
	private final DeyeSunHybridImpl parent;
	...
	public SurplusFeedInHandler(DeyeSunHybridImpl parent) {
		this.parent = parent;
	}

	protected Integer run(List<DeyeSunPv> chargers, Config config, ComponentManager componentManager) {
	 ...
	 * @return pv power
	 */
	private int getPvPower(List<DeyeSunPv> chargers) {
		var pvPower = 0;
	...
	private void applyPvPowerLimit(List<DeyeSunPv> chargers, Config config, boolean limitPv) {
		/*
	...
	private void setState(SurplusFeedInStateMachine state) {
		if (this.state != state) {
			...
			this.state = state;
			this.parent.channel(DeyeSunHybrid.ChannelId.SURPLUS_FEED_IN_STATE_MACHINE).setNextValue(state);
		}
	}
