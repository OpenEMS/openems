...
public class HeatingElementTest {
	...
	private static class MyConfig extends AbstractComponentConfig implements Config {
		...
		public MyConfig(String id, String inputChannelAddress, String outputChannelAddress1,
				String outputChannelAddress2, String outputChannelAddress3, String endtime, int powerOfPhase, Mode mode, Level level, 
	...
	public void test() throws OpenemsNamedException {
		...
		MyConfig myconfig = new MyConfig("ctrl1", ess0.toString(), output1.toString(), output2.toString(),
				output3.toString(), "15:45:00", 2000, Mode.AUTOMATIC, Level.LEVEL_0,  Priority.TIME, 1, 4000);
		...
		try {
			new ControllerTest(controller, componentManager, io)//
					.next(new TestCase() //
							...
							.output(output3, false)) // Grid active power : 0, Excess power : 0, from -> UNDEFINED
														// --to--> UNDEFINED, no of relais = 0
					.next(new TestCase() //
							.input(ess0, 0) //
							.output(output1, false) //
							.output(output2, false) //
					...
					.next(new TestCase() //
							...
							.output(output1, true) //
							.output(output2, false) //
							.output(output3, false)) // Grid active power : -2000, Excess power : 2000, from ->
														// UNDEFINED --to--> FIRST_PHASE, no of relais = 1
					.next(new TestCase() //
							.timeleap(clock, 15, ChronoUnit.MINUTES)//
							.input(ess0, -4000) //
							.output(output1, true) //
							.output(output2, true) //
							.output(output3, true)) // Grid active power : -4000, Excess power : 6000, from
													// ->FIRST_PHASE --to--> THIRD_PHASE, no of relais = 3
					.next(new TestCase() //
							.timeleap(clock, 15, ChronoUnit.MINUTES)//
							.input(ess0, -6000) //
							.output(output1, true) //
							.output(output2, true) //
							.output(output3, true)) // Grid active power : -6000, Excess power : 12000, from ->
													// THIRD_PHASE --to--> THIRD_PHASE, no of relais = 3
					.next(new TestCase() //
							.input(ess0, -7000) //
							.output(output1, true) //
							.output(output2, true) //
							.output(output3, true)) // Grid active power : -7000, Excess power : 13000, from ->
													// THIRD_PHASE --to--> THIRD_PHASE, no of relais = 3
					.next(new TestCase() //
							.timeleap(clock, 15, ChronoUnit.MINUTES)//
							.input(ess0, 0) //
							.output(output1, true) //
							.output(output2, true) //
							.output(output3, true)) // Grid active power : 0, Excess power : 6000, from -> THIRD_PHASE
													// --to--> THIRD_PHASE, no of relais = 3
					.next(new TestCase() //
							.timeleap(clock, 15, ChronoUnit.MINUTES)//
							.input(ess0, 1) //
							.output(output1, false) //
							.output(output2, false) //
							.output(output3, false)) // Grid active power : 1, Excess power : 0, from -> THIRD_PHASE
														// --to--> UNDEFINED, no of relais = 0
					.next(new TestCase() //
							...
							.output(output1, false) //
							.output(output2, false) //
							.output(output3, false)) // Grid active power : 20000, Excess power : 0, from ->
														// UNDEFINED--to--> UNDEFINED, no of relais = 0
					.next(new TestCase() //
							.timeleap(clock, 6, ChronoUnit.MINUTES)//
							.input(ess0, -4000) //
							.output(output1, true) //
							.output(output2, true) //
							.output(output3, false)) // Grid active power : -4000, Excess power : 10000, from
														// ->FIRST_PHASE --to--> THIRD_PHASE, no of relais = 3
