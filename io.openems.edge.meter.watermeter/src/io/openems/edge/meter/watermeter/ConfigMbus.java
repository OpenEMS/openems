...
@interface ConfigMbus {
    ...
    @AttributeDefinition(name = "Manual record position: Volume", description = "If \"Manual record position\" is selected: "
            + "Record position of metered volume in M-Bus Telegram.")
    int volAddress() default 0;

    @AttributeDefinition(name = "Manual record position: Timestamp", description = "If \"Manual record position\" is selected: "
            + "Record position of timestamp in M-Bus Telegram.")
    int timeStampAddress() default 1;

    @AttributeDefinition(name = "OpenEMS Timestamp", description = "Let OpenEMS create the timestamp instead of reading "
            + "it from the meter. Enable this if the meter does not have a timestamp.")
    ...
    @AttributeDefinition(name = "Polling interval in seconds", description = "If the \"Don't poll every second\" option is "
            + "turned on, this is the wait time between polling. Unit is seconds.")
