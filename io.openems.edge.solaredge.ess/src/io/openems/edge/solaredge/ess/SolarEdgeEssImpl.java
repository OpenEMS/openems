...
import io.openems.edge.solaredge.ess.common.SolarEdgeSunSpecModel.S103;
...
public class SolarEdgeEssImpl extends AbstractSunSpecEss implements SolarEdgeEss, ManagedSinglePhaseEss, SinglePhaseEss,
	...
	private static enum InverterType {
		SINGLE_PHASE(SolarEdgeSunSpecModel.S_101), //
		SPLIT_PHASE(SolarEdgeSunSpecModel.S_102), //
		THREE_PHASE(SolarEdgeSunSpecModel.S_103);

		private final List<SolarEdgeSunSpecModel> blocks;

		private InverterType(SolarEdgeSunSpecModel... blocks) {
	...
	private static final Map<SunSpecModel, Priority> ACTIVE_MODELS = ImmutableMap.<SunSpecModel, Priority>builder()
			...
			.put(SolarEdgeSunSpecModel.S_101, Priority.HIGH) // DefaultSunSpecModel without Events (Events are not supported by SolarEdge)
			.put(SolarEdgeSunSpecModel.S_102, Priority.HIGH) // DefaultSunSpecModel without Events (Events are not supported by SolarEdge)
			.put(SolarEdgeSunSpecModel.S_103, Priority.HIGH) // DefaultSunSpecModel without Events (Events are not supported by SolarEdge)
			.build();
	...
	/**
	 * Check if Battery Actual Power (=Charge/Discharge Power) has changed within the last 10 seconds.
	 */
	private boolean batteryActualPowerChanged() {
		...
		var pastBatteryPowerValues = batteryPowerChannel.getPastValues()
									.tailMap(LocalDateTime.now(this.componentManager.getClock()).minusSeconds(10), true); //
