...
public class ApplyPowerHandler {
	...
	public synchronized void apply(SolarEdgeEss solarEdge, int setActivePower, ControlMode controlMode,
		...
		if (gridActivePower.isDefined() && essActivePower.isDefined() && maxAcImport.isDefined()
			...
			apply = new ApplyPowerHandler.Result(CommandMode.AUTO, 0, 0);
		}
	...
	private static ApplyPowerHandler.Result calculate(SolarEdgeEss solarEdge, int activePowerSetPoint, int pvProduction,
		...
		case INTERNAL //
			-> handleInternalMode();
		case SMART //
			-> handleSmartMode(solarEdge, activePowerSetPoint, pvProduction, gridActivePower, essActivePower, maxAcImport,
					maxAcExport);
		case REMOTE //
	...
	private static Result handleInternalMode() {
		return new Result(CommandMode.AUTO, 0, 0);
	}
	...
	private static Result handleRemoteMode(int activePowerSetPoint, int pvProduction) {
		...
		if (activePowerSetPoint < 0) {
			return new Result(CommandMode.CHARGE_BAT, activePowerSetPoint * -1 + pvProduction, 0);
		}
		if (pvProduction >= activePowerSetPoint) {
			...
			return new Result(CommandMode.CHARGE_BAT, pvProduction - activePowerSetPoint, 0);

		} else {
			// Set-Point is positive && bigger than PV-Production -> feed all PV to grid +
			// discharge battery
			return new Result(CommandMode.DISCHARGE_BAT, 0, activePowerSetPoint - pvProduction);
