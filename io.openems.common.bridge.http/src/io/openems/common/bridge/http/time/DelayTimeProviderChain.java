...
public class DelayTimeProviderChain {
	...
	public static DelayTimeProviderChain fixedDelay(Duration duration) {
		final var delay = DelayTimeProvider.Delay.of(duration);
	...
	public static DelayTimeProviderChain fixedAtEveryFull(Clock clock, DurationUnit durationUnit) {
		return new DelayTimeProviderChain(() -> {
			...
			return DelayTimeProvider.Delay.of(Duration.between(now, now.truncatedTo(durationUnit) //
	 ...
	 * 
	 * @return a {@link DelayTimeProviderChain} which returns the
	 *         {@link DelayTimeProvider.Delay}
	 */
	public static DelayTimeProviderChain runNeverAgain() {
		return new DelayTimeProviderChain(DelayTimeProvider.Delay::infinite);
	}

	private final Supplier<DelayTimeProvider.Delay> supplier;

	public DelayTimeProviderChain(Supplier<DelayTimeProvider.Delay> supplier) {
		this.supplier = supplier;
	}

	public DelayTimeProvider.Delay getDelay() {
	 ...
	 * {@link DelayTimeProvider.Delay#plus(DelayTimeProvider.Delay)}.
	 * 
	...
	public static DelayTimeProviderChain plusFixedAmount(DelayTimeProviderChain origin, Duration duration) {
		return new DelayTimeProviderChain(() -> {
			return origin.getDelay().plus(DelayTimeProvider.Delay.of(duration));
		});
	}
	 ...
	 *           {@link #plusFixedAmount(DelayTimeProviderChain, Duration)}
	 * @param duration the {@link Duration} to add
	 * @return a {@link DelayTimeProviderChain} which returns the {@link Duration}
	 *         of the original {@link DelayTimeProviderChain} with the added amount
	 * @see #plusFixedAmount(DelayTimeProviderChain, Duration)
	 */
	 ...
	 * 
	 * @implNote delegates to
	 *           {@link #plusRandomDelay(DelayTimeProviderChain, int, TemporalUnit)}
	 * @param bound the upper bound (exclusive). Must be positive.
	 * @param unit  the {@link TemporalUnit} of the amount to add
	 * @return a {@link DelayTimeProviderChain} which returns the {@link Duration}
	 *         of the original {@link DelayTimeProviderChain} with the added amount
	 * @see #plusRandomDelay(DelayTimeProviderChain, int, TemporalUnit)
	 */
