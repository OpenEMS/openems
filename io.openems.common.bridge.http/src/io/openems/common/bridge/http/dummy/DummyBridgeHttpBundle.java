...
public class DummyBridgeHttpBundle {
	...
	public static class EndpointExpect {
		...
		private final CompletableFuture<BridgeHttp.Endpoint> request;

		public EndpointExpect(CompletableFuture<BridgeHttp.Endpoint> request) {
			super();
			this.request = request;
		}

		/**
		 * Creates a check for the current {@link BridgeHttp.Endpoint} request if it got
		 * called or not.
		 * 
		 * @return a {@link AtomicBoolean} which will be set to true once the
		 *         {@link BridgeHttp.Endpoint} got called
		 */
