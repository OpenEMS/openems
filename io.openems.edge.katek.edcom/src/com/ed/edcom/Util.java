...
public final class Util {
    ...
    public void setUserName(String name) {
        ...
        try {
            ...
            X509EncodedKeySpec spec2 = new X509EncodedKeySpec(
                    Base64.getDecoder().decode(sPub.getBytes("UTF-8")));
            Cipher chd = Cipher.getInstance("RSA");
            chd.init(Cipher.DECRYPT_MODE, kf.generatePublic(spec2));
            String plainText = new String(chd.doFinal(
                    Base64.getDecoder().decode(name.getBytes("UTF-8"))));
    ...
    public void setIndex(int a) {
        ...
        if (Float.floatToIntBits((float) Math.cos(Float.intBitsToFloat(this.a + 1937)
                * Math.PI * 0.3795f)) == a) {
            g = 1980;
        }
        this.a = 0;
