...
public class Undefined extends BaseState implements StateObject {
	...
	public IState getNextState() {
		...
		if (DecisionTableHelper.isWaitingForDevices(condition)) {
			if (getStateBefore() == null || Arrays.asList(OnOffGridState.START_SYSTEM, OnOffGridState.WAIT_FOR_DEVICES)
					.contains(getStateBefore())) {
		...
		if (DecisionTableHelper.isOnGridMode(condition)) {
			...
			if (getStateBefore() == null || Arrays
					.asList(OnOffGridState.START_SYSTEM, OnOffGridState.WAIT_FOR_DEVICES, OnOffGridState.ON_GRID_MODE)
					.contains(getStateBefore())) {
		...
		if (DecisionTableHelper.isOffGridMode(condition)) {
			if (getStateBefore() == null || Arrays.asList(OnOffGridState.ON_GRID_MODE, OnOffGridState.OFF_GRID_MODE,
					OnOffGridState.OFF_GRID_MODE_GRID_BACK, OnOffGridState.OFF_GRID_MODE_WAIT_FOR_GRID_AVAILABLE,
					OnOffGridState.OFF_GRID_MODE_ADJUST_PARMETER).contains(getStateBefore())) {
				return OnOffGridState.OFF_GRID_MODE;
			}
		}

		if (DecisionTableHelper.isOffGridGridBack(condition)) {
			if (getStateBefore() == null
					|| Arrays.asList(OnOffGridState.OFF_GRID_MODE, OnOffGridState.OFF_GRID_MODE_GRID_BACK)
							.contains(getStateBefore())) {
		...
		if (DecisionTableHelper.isOffGridWaitForGridAvailable(condition)) {
			if (getStateBefore() == null || Arrays.asList(OnOffGridState.OFF_GRID_MODE_WAIT_FOR_GRID_AVAILABLE,
					OnOffGridState.OFF_GRID_MODE_GRID_BACK).contains(getStateBefore())) {
