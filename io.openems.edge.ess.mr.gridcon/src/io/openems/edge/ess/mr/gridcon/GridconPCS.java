...
//		if (this.stateMachine.getState() != StateMachine.State.ONGRID) {
	...
	protected CCUState getCcuState() {
		...
		if (((BooleanReadChannel) this.channel(GridConChannelId.CCU_STATE_VOLTAGE_RAMPING_UP)).value()
		...
		if (((BooleanReadChannel) this.channel(GridConChannelId.CCU_STATE_DERATING_HARMONICS)).value()
				.asOptional().orElse(false)) {
