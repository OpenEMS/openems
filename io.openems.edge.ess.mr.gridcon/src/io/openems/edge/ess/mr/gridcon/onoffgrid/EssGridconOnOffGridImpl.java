...
public class EssGridconOnOffGridImpl extends EssGridcon
		implements EssGridconOnOffGrid, ManagedSymmetricEss, SymmetricEss, ModbusSlave, OpenemsComponent, EventHandler {
	...
	public EssGridconOnOffGridImpl() {
		super(EssGridconOnOffGrid.ChannelId.values());
	}

	@Activate
	private void activate(ComponentContext context, Config c) throws OpenemsNamedException {
		this.config = c;
		EssGridconOnOffGridImpl.super.activate(context, c.id(), c.alias(), c.enabled(), c.gridcon_id(), c.bms_a_id(),
	...
	protected void writeStateMachineToChannel() {
		this.channel(EssGridconOnOffGrid.ChannelId.STATE_MACHINE).setNextValue(this.mainStateObject.getState());
	}
