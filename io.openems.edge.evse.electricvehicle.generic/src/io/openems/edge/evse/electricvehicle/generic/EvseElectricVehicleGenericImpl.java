...
import io.openems.edge.evse.api.Phase;
...
public class EvseElectricVehicleGenericImpl extends AbstractOpenemsComponent
	...
	private void activate(ComponentContext context, Config config) {
		...
		if (config.maxCurrentSinglePhase() > MIN_CURRENT) {
			// TODO requires different Phase implementation for SINGLE/THREE.-
			limits.add(new Limit(Phase.L1, MIN_CURRENT, config.maxCurrentSinglePhase()));
		}
		if (config.maxCurrentThreePhase() > MIN_CURRENT) {
			limits.add(new Limit(Phase.ALL, MIN_CURRENT, config.maxCurrentThreePhase()));
		}
