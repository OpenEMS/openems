...
public class BrowserWebsocket
	...
	private OpenemsWebsocketService openemsWebsocketService;
	...
	/**
	 * Open event of websocket. Parses the Odoo "session_id" and stores it in a new
	 * Session.
	 */
	@Override
	protected void _onOpen(WebSocket websocket, ClientHandshake handshake) {
		...
		if (!data.getOdooSessionId().isPresent()) {
			...
			try {
				this.metadataService.getInfoWithSession(session.getData().getOdooSessionId().get());
			} catch (OpenemsException e) {
				error = e.getMessage();
			}
		}
		...
		if (error.isEmpty()) {
			...
			for (DeviceImpl device : data.getDevices()) {
				device.setOnline(this.openemsWebsocketService.isWebsocketConnected(device.getName()));
			}
		...
		if (deviceNameOpt.isPresent()) {
			...
			if (jMessage.has("historicData")) {
				...
				try {
					...
					JsonObject jReply = historicData(jMessageId, jHistoricData, deviceIdOpt,
							this.timedataService, Role.ADMIN);
			...
			if (jMessage.has("config")) {
				...
				if (jConfigOpt.isPresent()) {
					...
					if (configModeOpt.isPresent() && configModeOpt.get().equals("query")) {
						 ...
						 */
						Optional<JsonObject> openemsConfig = this.openemsWebsocketService.getConfig(deviceName);
	...
	private void forwardMessageToOpenems(BrowserSession session, WebSocket websocket, JsonObject jMessage,
		...
		this.openemsWebsocketService.send(deviceName, jMessage);
	}

	/**
	 * Handle current data subscriptions (copied from EdgeWebsocketHandler. Try to
	 * keep synced...)
	 *
	 * @param j
	 */
	...
	// * @param j
	// */
	// private synchronized void system(String deviceName, JsonElement
	// jSubscribeElement) {
	// JsonObject j = new JsonObject();
	...
	// log.warn("Trying to forward system call to [" + deviceName + "], but it is
	// not online");
	// }
	// WebSocket openemsWebsocket = openemsWebsocketOpt.get();
	// log.info(deviceName + ": forward system call to OpenEMS " +
	// StringUtils.toShortString(j, 100));
	 ...
	 * @param j
	 */
	private JsonObject historicData(JsonArray jMessageId, JsonObject jHistoricData, Optional<Integer> deviceId,
			TimedataService timedataService, Role role) {
		try {
			...
			if (mode.equals("query")) {
				...
				JsonArray jData = timedataService.queryHistoricData(deviceId, fromDate, toDate, channels, resolution);
				// send reply
	...
	public void setOpenemsWebsocketService(OpenemsWebsocketService openemsWebsocketService) {
		this.openemsWebsocketService = openemsWebsocketService;
	}

	public void setTimedataService(TimedataService timedataService) {
