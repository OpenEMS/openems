...
public class WagoImpl extends AbstractOpenemsModbusComponent
		implements DigitalOutput, DigitalInput, ModbusComponent, OpenemsComponent {

	private static final int UNIT_ID = 1;

	private final Logger log = LoggerFactory.getLogger(WagoImpl.class);
	...
	private void activate(ComponentContext context, Config config) throws OpenemsException {
		...
		this.configFuture = this.configExecutor.schedule(() -> {
			try {
				var doc = WagoImpl.downloadConfigXml(this.ipAddress, config.username(), config.password());
