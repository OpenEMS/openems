...
*For KOSTAL PIKO*:
* Network connectivity to PIKO inverter
* Known IP address and port of inverter

*For KOSTAL Plenticore*:
* Modbus TCP bridge configured and running (e.g., `io.openems.edge.bridge.modbus`)
* Modbus connectivity to Plenticore inverter and KSEM (if used)
* Proper Modbus Unit IDs configured on devices

== Known Limitations

* PIKO components use REST API communication - HTTP connectivity required
* Plenticore components use Modbus - proper Modbus configuration required
* Read-only mode recommended for safety - read-write mode requires careful consideration
* Battery SoC minimum and watchdog settings are important for system stability
* KSEM can read from grid, inverter, or battery depending on configuration
