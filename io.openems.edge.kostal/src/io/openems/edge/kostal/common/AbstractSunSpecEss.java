...
public abstract class AbstractSunSpecEss extends AbstractOpenemsSunSpecComponent
		implements
		ManagedSymmetricEss, SymmetricEss, OpenemsComponent {

	private final Logger log = LoggerFactory
			.getLogger(AbstractSunSpecEss.class);
	...
	public String debugLog() {
		return new StringBuilder() //
				...
				.append("|ESS ActivePower:")
				.append(this.getActivePower().asString()) //
				.toString();
	}

	@Override
	protected <T extends Channel<?>> Optional<T> getSunSpecChannel(
			SunSpecPoint point) {
		return super.getSunSpecChannel(point);
	}

	@Override
	protected <T extends Channel<?>> T getSunSpecChannelOrError(
			SunSpecPoint point) throws OpenemsException {
