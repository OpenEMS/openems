...
Choose the appropriate meter type based on what the Smart Meter measures:

* **GRID** - Measures grid connection point (typical for monitoring building load)
* **PRODUCTION** - Measures solar generation (if placed at PV output)
* **CONSUMPTION** - Measures consumption from the meter

== Integration Architecture

Typical Fronius installation:

```
PV Array
   ↓
Fronius Symo Inverter ← Modbus → OpenEMS
   ↓
Grid/Building
   ↓
Fronius Smart Meter ← Modbus → OpenEMS
```

Use cases:
* Monitor solar production: Configure PV inverter component with read-only
* Measure grid power: Configure Smart Meter as GRID type
* Limit solar feed-in: Enable managed mode on PV inverter in ES controller
* Load balancing: Use both components for complete energy visibility

== Related Bundles

* xref:../../io.openems.edge.bridge.modbus/readme.adoc[Modbus Bridge] - Required for communication
* xref:../../io.openems.edge.pvinverter.api/readme.adoc[PV Inverter API] - Interface definition
* xref:../../io.openems.edge.meter.api/readme.adoc[Meter API] - Meter interface definition
* xref:../../io.openems.edge.controller.pvinverter.selltogridlimit/readme.adoc[Sell-to-Grid Limit] - Example controller

== External Resources

* Fronius Website: https://www.fronius.com/[Fronius icon:external-link[]]
* SunSpec Alliance: https://sunspec.org/[SunSpec icon:external-link[]]
* Modbus SunSpec Protocol: https://sunspec.org/sunspec-modbus-standard/[SunSpec Modbus Standard icon:external-link[]]
