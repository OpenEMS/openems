...
public class OdooHandler {
	...
	if (userRole.equals(OdooUserRole.OWNER)) {
	    fields.add(new FieldValue<>(Field.EdgeDeviceUserRole.OFFLINE_ALERT_TIME_TO_WAIT, 1440));
		fields.add(new FieldValue<>(Field.EdgeDeviceUserRole.SUM_STATE_ALERT_TIME_TO_WAIT, 1440));
		fields.add(new FieldValue<>(Field.EdgeDeviceUserRole.SUM_STATE_ALERT_LEVEL, 3));
	}
	...
	var edgeUserFields = new Field[] { //
		...
		Field.EdgeDeviceUserRole.OFFLINE_ALERT_TIME_TO_WAIT, //
		Field.EdgeDeviceUserRole.LAST_NOTIFICATION, //
		Field.EdgeDeviceUserRole.SUM_STATE_ALERT_TIME_TO_WAIT, //
		Field.EdgeDeviceUserRole.SUM_STATE_ALERT_NOTIFICATION, //
		Field.EdgeDeviceUserRole.SUM_STATE_ALERT_LEVEL //
	};
	// Define Domains
	...
	for (var edgeUser : edgeUsers) {
		...
		if (usersMap.containsKey(userOdooId)) {
		    ...
		    var offlineAlertTimeToWait = OdooUtils.getAs(Field.EdgeDeviceUserRole.OFFLINE_ALERT_TIME_TO_WAIT,
			    edgeUser, Integer.class);
		    var offlineAlertLastNotificationStr = OdooUtils
			    .getAsOrElse(Field.EdgeDeviceUserRole.LAST_NOTIFICATION, edgeUser, String.class, null);
		    var offlineAlertLastNotification = OdooUtils.DateTime
			    .stringToDateTime(offlineAlertLastNotificationStr);
		    var sumStateAlertDelayTime = OdooUtils.getAs(Field.EdgeDeviceUserRole.SUM_STATE_ALERT_TIME_TO_WAIT,
			    edgeUser, Integer.class);
		    var sumStateAlertLastNotificationStr = OdooUtils.getAsOrElse(
			    Field.EdgeDeviceUserRole.SUM_STATE_ALERT_NOTIFICATION, edgeUser, String.class, null);
		    var sumStateAlertLastNotification = OdooUtils.DateTime
			    .stringToDateTime(sumStateAlertLastNotificationStr);
		    var sumStateAlertLevelInt = OdooUtils.getAs(Field.EdgeDeviceUserRole.SUM_STATE_ALERT_LEVEL,
			    edgeUser, Integer.class);
		    var sumStateAlertLevel = Level.fromValue(sumStateAlertLevelInt).get();

		    result.add(new AlertingSetting(edgeUserId, login, Role.getRole(role), offlineAlertLastNotification,
			    offlineAlertTimeToWait, sumStateAlertLastNotification,
			    sumStateAlertDelayTime, sumStateAlertLevel));
		}
	    }
	}
	return result;
	...
	for (var setting : userAlertingSettings) {
		...
		OdooUtils.write(this.credentials, Field.EdgeDeviceUserRole.ODOO_MODEL, new Integer[] { edgeUsers[0] }, //
			new FieldValue<>(Field.EdgeDeviceUserRole.OFFLINE_ALERT_TIME_TO_WAIT,
				setting.getOfflineAlertDelayTime()), //
			new FieldValue<>(Field.EdgeDeviceUserRole.SUM_STATE_ALERT_TIME_TO_WAIT,
				setting.getSumStateAlertDelayTime()), //
			new FieldValue<>(Field.EdgeDeviceUserRole.SUM_STATE_ALERT_LEVEL,
				setting.getSumStateAlertLevel()) //
		);
	    } else {
		...
		OdooUtils.create(this.credentials, Field.EdgeDeviceUserRole.ODOO_MODEL, //
			...
			new FieldValue<>(Field.EdgeDeviceUserRole.OFFLINE_ALERT_TIME_TO_WAIT,
				setting.getOfflineAlertDelayTime()), //
			new FieldValue<>(Field.EdgeDeviceUserRole.LAST_NOTIFICATION,
				setting.getOfflineAlertLastNotification()), //
			new FieldValue<>(Field.EdgeDeviceUserRole.SUM_STATE_ALERT_TIME_TO_WAIT,
				setting.getSumStateAlertDelayTime()), //
			new FieldValue<>(Field.EdgeDeviceUserRole.SUM_STATE_ALERT_NOTIFICATION,
				setting.getSumStateAlertLastNotification()), //
			new FieldValue<>(Field.EdgeDeviceUserRole.SUM_STATE_ALERT_LEVEL,
				setting.getSumStateAlertLevel()) //
		);
