...
describe("History Production Details - _sum", () => {
    const defaultEMS = DUMMY_CONFIG.FROM(
        DUMMY_CONFIG.COMPONENT.SUM("_sum", "Gesamt"),
        DUMMY_CONFIG.COMPONENT.SOLAR_EDGE_PV_INVERTER("meter0"),
    );
    ...
    it("#getChartData() - essDcCharger configured", () => {
        {
            const defaultEMS = DUMMY_CONFIG.FROM(
                DUMMY_CONFIG.COMPONENT.SUM("_sum", "Gesamt"),
                DUMMY_CONFIG.COMPONENT.GOODWE_CHARGER_MPPT_TWO_STRING("charger0"),
            );
            expectView(defaultEMS, TEST_CONTEXT, "line", HISTORY.DAY,
                {
                    datasets: {
