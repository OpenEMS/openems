...
export class ScheduleStateAndPriceChartReducedComponent extends AbstractHistoryChart implements OnInit, OnChanges, OnDestroy {
    ...
    protected override updateChart() {
        ...
        this.edge.sendRequest(
            ...
            //console.log("Raw Schedule Data:", schedule);  // LOG THE DATA
            //console.log("Extracted Prices:", schedule.map(entry => entry.price)); // Check if prices are available

            if (!schedule || schedule.length === 0) {
                console.warn("No data received for schedule!");
                return;
            }
            ...
            this.datasets = scheduleChartDataReduced.datasets;
            this.loading = false;
            this.setLabel();
            this.stopSpinner();
    ...
    private applyControllerSpecificOptions() {
        ...
        // Ensure that the right Y-axis is completely removed
        if (this.options.scales[ChartAxis.RIGHT]) {
            delete this.options.scales[ChartAxis.RIGHT];
        }
        if (this.options.scales[ChartAxis.RIGHT_2]) {
            delete this.options.scales[ChartAxis.RIGHT_2];
        }
    }


    private applyControllerSpecificOptionsNew() {
