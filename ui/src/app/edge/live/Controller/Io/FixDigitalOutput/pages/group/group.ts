...
@Component({
    selector: "oe-fix-digital-output-group",
    standalone: true,
...
export class ControllerFixDigitalOutputGroupComponent implements OnInit {
    ...
    public async ngOnInit(): Promise<void> {
        ...
        this.components = config?.getComponentIdsByFactory("Controller.Io.FixDigitalOutput")
    ...
    public navigateTo(componentId: string): void {
        this.service.getCurrentEdge().then(async edge => {
            this.router.navigate(["/device", edge.id, "live", "controller", "io-fix-digital-output", componentId]);
        });
    }
}
