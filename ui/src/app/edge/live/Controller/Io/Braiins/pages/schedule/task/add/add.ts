...
type PayloadMode = NonNullable<
    Parameters<ControllerBraiinsManualPayload["setValue"]>[0]
>["mode"];
...
export class ControllerBraiinsAddTaskComponent extends JsCalendarAddTaskComponent {
    public payload = model<ControllerBraiinsManualPayload>(
        new ControllerBraiinsManualPayload(),
    );
    public allowedPeriods = model<
        TSignalValue<TaskFormComponent["allowedPeriods"]>
    >(["daily", "weekly", "monthly"]);
    protected modeOptions: ModeOption[] = Object.values(
        ControllerBraiinsShared.Mode,
    ).map((mode) => ({
        value: mode,
        label: ControllerBraiinsShared.CONVERT_TO_MODE_LABEL(this.translate)(
            mode,
        ),
    }));
