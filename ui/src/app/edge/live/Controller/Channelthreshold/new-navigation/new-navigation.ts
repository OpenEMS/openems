        ...
        if (outputChannelAddress != null) {
            lines.push({
                ...
                value: (currentData: CurrentData) => {
                    const outputChannelAddress = component.getPropertyFromComponent<string>("outputChannelAddress");
                    const channel = CurrentDataUtils.getChannel(
                        ChannelAddress.fromStringSafely(outputChannelAddress),
                        currentData.allComponents,
                    ) as string;

                    return Converter.ON_OFF(translate)(channel);
                },
                channelsToSubscribe: [ChannelAddress.fromString(outputChannelAddress)],
            });
        }
