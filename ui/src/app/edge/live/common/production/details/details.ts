...
export class CommonProductionDetailsComponent extends AbstractFormlyComponent {
    ...
    protected override generateView(): OeFormlyView {
        ...
        if (productionMeters?.length === 1) {
            lines.push(
                { type: "advanced-electricity-meter-line", component: productionMeters[0] },
                { type: "horizontal-line" },
            );
        }
        ...
        if (productionMeters?.length > 1) {
            lines.push(
                {
                    ...
                    name: this.translate.instant("GENERAL.TOTAL") + (chargerComponents.length > 0 ? " AC" : ""),
                    converter: Converter.POWER_IN_WATT,
                },
                ...Phase.THREE_PHASE.map(
                    (phase) =>
                        <OeFormlyField>{
                            type: "channel-line",
            ...
            for (const meter of productionMeters) {
                lines.push({ type: "advanced-electricity-meter-line", component: meter }, { type: "horizontal-line" });
            }
        }
        ...
        for (const component of chargerComponents) {
            lines.push({ type: "horizontal-line" }, { type: "advanced-ess-charger-line", component: component });
        }
