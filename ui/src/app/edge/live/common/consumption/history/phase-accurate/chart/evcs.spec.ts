...
describe("History Consumption Details - evcs", () => {
    const defaultEMS = DummyConfig.from(DummyConfig.Component.EVCS_KEBA_KECONTACT("evcs0", "Charging Station"));

    let TEST_CONTEXT: TestContext & { route: ActivatedRoute };
    beforeEach(async () => {
        TEST_CONTEXT = await TestingUtils.setupWithActivatedRoute("evcs0");
    });
...
export function expectView(
    config: EdgeConfig,
    testContext: TestContext & { route: ActivatedRoute },
    chartType: "line" | "bar",
