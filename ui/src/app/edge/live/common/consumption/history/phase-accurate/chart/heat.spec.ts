...
describe("History Consumption Details - heat", () => {
    const defaultEMS = DummyConfig.from(DummyConfig.Component.HEAT_MYPV_ACTHOR("heat0", "Heating Element"));

    let TEST_CONTEXT: TestContext & { route: ActivatedRoute };
    beforeEach(async () => {
        TEST_CONTEXT = await TestingUtils.setupWithActivatedRoute("heat0");
    });
...
export function expectView(
    ...
    expect(
        TestingUtils.removeFunctions(
            OeChartTester.apply(
                HeatChartDetailComponent.getChartData(
