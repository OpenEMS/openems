...
import {EdgesCurrentDataNotification} from '../jsonrpc/notification/edgesCurrentDataNotification';
...
export class Websocket {
    ...
    private onNotification(message: JsonrpcNotification): void {
        switch (message.method) {
            ...
            case EdgesCurrentDataNotification.METHOD:
                this.handleEdgesCurrentDataNotification(message as EdgesCurrentDataNotification);
                break;
        }
    }
    ...
    private handleEdgeConfigNotification(edgeId: string, message: EdgeConfigNotification): void {
        const edges = this.service.edges.getValue();
    ...
    /**
     * TODO This method is just a quick and dirty way of handling the new rpc message "edgesCurrentData"
     * @param message the message containing all edge and channel information
     */
    private handleEdgesCurrentDataNotification(message: EdgesCurrentDataNotification) {
        Object.keys(message.params).filter(key => key === this.service.currentEdge.value.id).forEach(edgeId => {
            const edges = this.service.edges.getValue();
            if (edgeId in edges) {
                const edge = edges[edgeId];
                edge.currentData.next(new CurrentData(message.params[edgeId]));
            }
        });
    }
}
