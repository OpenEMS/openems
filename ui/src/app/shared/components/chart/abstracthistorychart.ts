...
/**
 * Declare custom Tooltip Positioner "bottom". See example at
 * https://www.chartjs.org/docs/latest/samples/tooltip/position.html
 */
declare module "chart.js" {
    interface TooltipPositionerMap {
        bottom: Chart.TooltipPositionerFunction<Chart.ChartType>;
    }
}
Chart.Tooltip.positioners["bottom"] = function (elements, eventPosition) {
    const chartArea = this.chart.chartArea;
    const eventX = eventPosition.x ?? (chartArea.left + chartArea.right) / 2;

    const isRightHalf = eventX > (chartArea.left + chartArea.right) / 2;
    const padding = 20;
    const x = isRightHalf ? eventX - padding : eventX + padding;
...
{
    protected static readonly phaseColors: string[] = [
        "rgb(255,127,80)",
        "rgb(91, 92, 214)",
        "rgb(128,128,0)",
    ];
    ...
    /**
     * TODO: workaround with Observables, to not have to pass the period on
     * Initialisation
     */
    ...
    @Output() public setChartConfig: EventEmitter<ChartTypes.ChartConfig> =
        new EventEmitter();
    ...
    protected channelData: { data: { [name: string]: number[] } } = {
        data: {},
    };
     ...
     * @param energyPeriodResponse The response of a
     *   {@link QueryHistoricTimeseriesEnergyPerPeriodRequest} or
     *   {@link QueryHistoricTimeseriesDataResponse}
     * @param energyResponse The response of a
     *   {@link QueryHistoricTimeseriesEnergyResponse}
     */
    public static fillChart(
        chartType: "line" | "bar",
        chartObject: HistoryUtils.ChartData,
        ...
        const channelData: { data: { [name: string]: number[] } } = {
            data: {},
        };
        ...
        const displayValues: HistoryUtils.DisplayValue<HistoryUtils.CustomOptions>[] =
            chartObject.output(channelData.data, labels);
        const legendOptions: {
            label: string;
            strokeThroughHidingStyle: boolean;
            hideLabelInLegend: boolean;
        ...
        displayValues.forEach((displayValue, index) => {
            ...
            if (
                ...
                displayValue.nameSuffix(energyResponse) != null
            ) {
                nameSuffix = displayValue.nameSuffix(energyResponse);
            }

            const yAxis = chartObject.yAxes.find(
                (yaxis) =>
                    yaxis?.yAxisId ==
                    (displayValue?.yAxisId ?? chartObject.yAxes[0]?.yAxisId),
            );

            // Filter existing values
            if (displayValue) {
                const label = AbstractHistoryChart.getTooltipsLabelName(
                    displayValue.name,
                    yAxis?.unit,
                    nameSuffix,
                );
                ...
                const configuration = AbstractHistoryChart.fillData(
                    displayValue,
                    label,
                    chartObject,
                    chartType,
    ...
    public static fillData(
        element: HistoryUtils.DisplayValue<HistoryUtils.CustomOptions>,
        label: string,
        chartObject: HistoryUtils.ChartData,
        chartType: "line" | "bar",
        data: number[] | null,
        ...
        if (Array.isArray(element.stack)) {
            for (const stack of element.stack) {
                datasets.push(
                    AbstractHistoryChart.getDataSet(
                        element,
                        label,
                        normalizedData,
                        stack,
                        chartObject,
                        element.custom?.type ?? chartType,
                    ),
                );
                legendOptions.push(
                    AbstractHistoryChart.getLegendOptions(label, element),
                );
            }
        } else {
            datasets.push(
     ...
     * @returns The label, the hidingStyle of the legendLabel:
     *   strokeThroughHidingStyle, hideLabelInLegend
     */
    public static getLegendOptions(
        label: string,
        element: HistoryUtils.DisplayValue<HistoryUtils.CustomOptions>,
    ): {
        label: string;
    ...
    public static getColors(
        ...
        return {
            backgroundColor:
                "rgba(" +
                (chartType == "bar"
                    ? color.split("(").pop().split(")")[0] + ",0.7)"
                    : color.split("(").pop().split(")")[0] + ",0.05)"),
     ...
     */
    public static applyChartTypeSpecificOptionsChanges(
        chartType: "bar" | "line",
        options: Chart.ChartOptions,
        service: Service,
        chartObject: HistoryUtils.ChartData | null,
    ): Chart.ChartOptions {
        switch (chartType) {
            case "bar": {
    ...
    public static getDataSet(
        ...
        const dataset: Chart.ChartDataset = {
            ...
            yAxisID:
                element.yAxisId != null
                    ? element.yAxisId
                    : chartObject.yAxes.find(
                          (element) => element.yAxisId == ChartAxis.LEFT,
                      )?.yAxisId,
            order: element.order ?? Number.MAX_VALUE,
            maxBarThickness: 100,
            borderWidth: 2,
            ...(stack != null ? { stack: stack.toString() } : {}),
            ...(element.borderDash != null
                ? { borderDash: element.borderDash }
                : {}),
    ...
    public static getOptions(
        ...
        let options: Chart.ChartOptions = Utils.deepCopy(
            <Chart.ChartOptions>(
                Utils.deepCopy(
                    AbstractHistoryChart.getDefaultXAxisOptions(
                        chartOptionsType,
                        service,
                        labels,
                    ),
        ...
        options.plugins.tooltip.callbacks.labelPointStyle = function (context: {
            dataset: Chart.ChartDataset;
        }) {
            return ChartConstants.Plugins.ToolTips.POINT_STYLE(context.dataset);
        };

        options.plugins.tooltip.callbacks.title = (
            tooltipItems: Chart.TooltipItem<any>[],
        ): string => {
            if (tooltipItems?.length === 0) {
                return null;
            }
            return AbstractHistoryChart.toTooltipTitle(
                service.historyPeriod.value.from,
                service.historyPeriod.value.to,
                tooltipItems[0]?.label,
                service,
                chartOptionsType,
            );
        };
        options = AbstractHistoryChart.applyChartTypeSpecificOptionsChanges(
            chartType,
            options,
            service,
            chartObject,
        ...
        options.plugins.tooltip.callbacks.label = (
            ...
            const displayValue = displayValues.find(
                (element) => element.name === label?.split(":")?.[0],
            );
            ...
            const unit =
                chartObject.yAxes?.find(
                    (el) => el.yAxisId === displayValue.yAxisId,
                )?.unit ?? chartObject.yAxes[0]?.unit;
            ...
            if (unit != null) {
                tooltipsLabel = AbstractHistoryChart.getToolTipsAfterTitleLabel(
                    unit,
                    chartType,
                    value,
                    translate,
            ...
            return AbstractHistoryChart.getToolTipsSuffix(
                label,
                value,
                displayValue.custom?.formatNumber ??
                    chartObject.tooltip.formatNumber,
                unit,
                chartType,
                translate,
        ...
        options.plugins.tooltip.callbacks.labelColor = (
            item: Chart.TooltipItem<any>,
        ) => {
            ...
            return {
                ...
                backgroundColor: ColorUtils.changeOpacityFromRGBA(
                    backgroundColor,
                    1,
                ),
            };
        ...
        options.plugins.legend.labels.generateLabels = function (
            ...
            chart.data.datasets.forEach(
                (dataset: Chart.ChartDataset, index) => {
                    const legendItem = legendOptions?.find(
                        (element) => element.label == dataset.label,
                    );
                    //Remove duplicates like 'directConsumption' from legend
                    if (
                        chartLegendLabelItems.filter(
                    ...
                    const isHidden =
                        legendItem?.strokeThroughHidingStyle ?? null;
                    ...
                    const currentDisplayValue = displayValues.find(
                        (el) =>
                            el.name ==
                            chartLegendLabelItem.text?.split(":")?.[0],
                    );
                    ...
                    if (currentDisplayValue?.custom) {
                        ...
                        chart.options =
                            AbstractHistoryChart.activateOrDeactivatePlugin(
                                currentDisplayValue,
                                options,
                                chartType,
                                show,
        ...
        options.plugins.tooltip.callbacks.afterTitle = function (
            items: Chart.TooltipItem<any>[],
        ) {
            ...
            const afterTitle =
                typeof chartObject.tooltip?.afterTitle == "function"
                    ? chartObject.tooltip?.afterTitle(stack)
                    : null;

            const cumulatedValue = datasets
                .filter((el) => el.stack == stack)
                .reduce(
                    (_total, dataset) =>
                        Utils.addSafely(
                            _total,
                            Math.abs(dataset.data[datasetIndex]),
                        ),
                    0,
                );
            const unit =
                chartObject.yAxes?.find((el) => el.yAxisId === yAxisId)?.unit ??
        ...
        options.plugins.legend.onClick = function (
            ...
            legendItems.forEach((item) => {
                ...
                const currentDisplayValue = displayValues.find(
                    (el) => el.name == legendItem.text.split(":")[0],
                );
                ...
                meta.hidden =
                    meta.hidden === null
                        ? !chart.data.datasets[item.index].hidden
                        : null;
            });

            function showOrHideYAxis(
                datasets: Chart.ChartDataset[],
                chart: Chart.Chart,
            ) {
                for (const key of Object.keys(
                    ObjectUtils.excludeProperties(chart.options.scales, ["x"]),
                )) {
                    const axisDatasets = datasets.filter(
                        (d) => d["yAxisID"] === key,
                    );
                    // It is important to update the options in the outer scope!
                    options.scales[key].display = axisDatasets.some(
                        (d) => !d.hidden,
                    );
                }
            }
        ...
        options.scales.x.ticks.color = getComputedStyle(
            document.documentElement,
     ...
     */
    public static getYAxisOptions(
        options: Chart.ChartOptions,
        element: HistoryUtils.yAxes,
        translate: TranslateService,
        chartType: "line" | "bar",
        datasets: ChartTypes.Dataset[],
        showYAxisType?: boolean,
        formatNumber?: HistoryUtils.ChartData["tooltip"]["formatNumber"],
    ): Chart.ChartOptions {
        const locale: string = Language.geti18nLocale();
        const baseConfig = ChartConstants.DEFAULT_Y_SCALE_OPTIONS(
            element,
            translate,
            chartType,
            datasets,
                    ...
                    ticks: {
                        ...
                        callback: function (value, index, values) {
                            if (typeof value === "number") {
                                return TimeUtils.formatSecondsToDuration(
                                    value,
                                    locale,
                                );
                            }
     ...
     * @returns A string, that is either the baseName, if no suffix is provided,
     *   or a baseName with a formatted number
     */
    public static getTooltipsLabelName(
        baseName: string,
        unit: YAxisType,
                ...
                switch (unit) {
                    case YAxisType.ENERGY:
                        return (
                            baseName +
                            ": " +
                            formatNumber(suffix / 1000, locale, "1.0-1") +
                            " kWh"
                        );
                    case YAxisType.PERCENTAGE:
                        return (
                            baseName +
                            ": " +
                            formatNumber(suffix, locale, "1.0-1") +
                            " %"
                        );
                    case YAxisType.RESTRICTION:
                    case YAxisType.RELAY:
                    ...
                    case YAxisType.ENERIX_CONTROL:
                    case YAxisType.TIME: {
                        const pipe = new FormatSecondsToDurationPipe(
                            new DecimalPipe(Language.DE.key),
                        );
                    ...
                    case YAxisType.TEMPERATURE:
                        return (
                            baseName +
                            ": " +
                            formatNumber(suffix / 10, locale, "1.0-1") +
                            " °C"
                        );
                    default:
                        return baseName;
    ...
    public static getToolTipsSuffix(
        ...
        switch (title) {
            ...
            case YAxisType.HEAT_PUMP:
                return (
                    prefix +
                    ": " +
                    ChartConstants.Plugins.ToolTips.HEAT_PUMP_SUFFIX(
                        translate,
                        value,
                    )
                );
            case YAxisType.ENERIX_CONTROL:
                return (
                    prefix +
                    ": " +
                    ChartConstants.Plugins.ToolTips.ENERIX_CONTROL_SUFFIX(
                        translate,
                        value,
                    )
                );
            ...
            case YAxisType.CURRENCY: {
                ...
                suffix = Currency.getCurrencyLabelByCurrency(currency);
                break;
            }
            case YAxisType.RESTRICTION:
            case YAxisType.PERCENTAGE:
                suffix = AbstractHistoryChart.getToolTipsAfterTitleLabel(
                    title,
                    chartType,
                    value,
                    translate,
        ...
        return (
            prefix + ": " + formatNumber(value, locale, format) + " " + suffix
        );
    }
     ...
     */
    public static getDefaultXAxisOptions(
        xAxisType: XAxisType,
        service: Service,
        labels: ChartTypes.Label[],
    ): Chart.ChartOptions {
        ...
        switch (xAxisType) {
            ...
            case XAxisType.TIMESERIES:
                options = <Chart.ChartOptions>(
                    Utils.deepCopy(DEFAULT_TIME_CHART_OPTIONS())
                );
                options.scales.x["time"].unit = calculateResolution(
                    service,
                    service.historyPeriod.value.from,
                    service.historyPeriod.value.to,
                ).timeFormat;
     ...
     */
    protected static toTooltipTitle(
        fromDate: Date,
        toDate: Date,
        label: string,
        service: Service,
        ...
        const unit = calculateResolution(service, fromDate, toDate).resolution
            .unit;
     ...
     */
    protected static removePlugins(
        options: Chart.ChartOptions,
    ): Chart.ChartOptions {
     ...
     */
    protected static getToolTipsAfterTitleLabel(
        title: YAxisType | null,
        chartType: "bar" | "line",
     ...
     */
    private static activateOrDeactivatePlugin(
        displayValue: HistoryUtils.DisplayValue<HistoryUtils.CustomOptions>,
        options: Chart.ChartOptions,
        chartType: "line" | "bar",
        show: boolean,
    ): Chart.ChartOptions {
        ...
        switch (displayValue.custom["pluginType"]) {
            case "box":
                if (
                    (displayValue.custom as HistoryUtils.BoxCustomOptions)
                    ...
                    options.plugins["annotation"] = {
                        annotations: (
                            ...
                            ...annotation,
                            ...AbstractHistoryChart.getColors(
                                displayValue.color,
                                chartType,
                            ),
                            display: show,
            ...
            case "datalabels":
                options.plugins["datalabels"] =
                    ChartConstants.Plugins.BAR_CHART_DATALABELS(
                        (
                            displayValue.custom as HistoryUtils.DataLabelsCustomOptions
                        ).datalabels.displayUnit,
                        true,
                    );
                Chart.Chart.register(
                    ChartConstants.Plugins.BAR_CHART_DATALABELS("kWh", true)
                        .plugin,
                );
                break;
        }
    ...
    protected getChartHeight(): number | null {
        return ViewUtils.getChartContentHeightInVh(
            this.navigationService.position(),
        );
    }
    ...
    /**
     * Sends the Historic Timeseries Data Query and makes sure the result is not
     * empty.
     *
     * @param fromDate The From-Date
     * @param toDate The To-Date
     ...
     */
    protected queryHistoricTimeseriesData(
        fromDate: Date,
        toDate: Date,
        res?: Resolution,
        ...
        const resolution =
            res ??
            calculateResolution(this.service, fromDate, toDate).resolution;

        return new Promise<QueryHistoricTimeseriesDataResponse>(
            (resolve, reject) => {
                this.service.getCurrentEdge().then((edge) =>
                    this.service.getConfig().then(async () => {
                        const channelAddresses = (
                            await this.getChannelAddresses()
                        ).powerChannels;
                        ...
                        edge.sendRequest(this.service.websocket, request)
                            .then((response) => {
                                const result = (
                                    response as QueryHistoricTimeseriesDataResponse
                                )?.result;
                                ...
                                if (Object.keys(result).length !== 0) {
                                    ...
                                    this.errorResponse =
                                        new JsonrpcResponseError(request.id, {
                                            code: 1,
                                            message: "Empty Result",
                                        });
                                    responseToReturn =
                                        new QueryHistoricTimeseriesDataResponse(
                                            response.id,
                                            {
                                                timestamps: [null],
                                                data: { null: null },
    ...
    /**
     * Sends the Historic Timeseries Energy per Period Query and makes sure the
     * result is not empty. Symbolizes First substracted from last Datapoint for
     * each period, only used for cumulated channel
     *
     * @param fromDate The From-Date
     * @param toDate The To-Date
     */
    protected queryHistoricTimeseriesEnergyPerPeriod(
        ...
        resolution: ReturnType<
            typeof calculateResolution
        >["resolution"] = calculateResolution(this.service, fromDate, toDate)
            .resolution,
    ): Promise<QueryHistoricTimeseriesEnergyPerPeriodResponse | null> {
        this.isDataExisting = true;
        const result: Promise<QueryHistoricTimeseriesEnergyPerPeriodResponse> =
            new Promise<QueryHistoricTimeseriesEnergyPerPeriodResponse>(
                (resolve, reject) => {
                    this.service.getCurrentEdge().then((edge) => {
                        this.service.getConfig().then(async () => {
                            const channelAddresses = (
                                await this.getChannelAddresses()
                            ).energyChannels.filter(
                                (element) => element != null,
                            );
                            const request =
                                new QueryHistoricTimeseriesEnergyPerPeriodRequest(
                                    DateUtils.maxDate(
                                        fromDate,
     ...
     * @param toDate The To-Date
     */
    protected queryHistoricTimeseriesEnergy(
        fromDate: Date,
        toDate: Date,
    ): Promise<QueryHistoricTimeseriesEnergyResponse> {
        this.isDataExisting = true;

        const result: Promise<QueryHistoricTimeseriesEnergyResponse> =
            new Promise<QueryHistoricTimeseriesEnergyResponse>(
                (resolve, reject) => {
                    this.service.getCurrentEdge().then((edge) => {
                        this.service.getConfig().then(async () => {
                            const channelAddresses =
                            ...
                            const request =
                                new QueryHistoricTimeseriesEnergyRequest(
                                    DateUtils.maxDate(
                                        fromDate,
                                        edge?.firstSetupProtocol,
                                    ),
                            ...
                            if (channelAddresses.length > 0) {
                                edge.sendRequest(
                                    ...
                                    .then((response) => {
                                        const result = (
                                            response as QueryHistoricTimeseriesEnergyResponse
                                        )?.result;
                                        if (Object.keys(result).length != 0) {
                                            resolve(
                                                response as QueryHistoricTimeseriesEnergyResponse,
    ...
    protected setChartLabel() {
        this.options = AbstractHistoryChart.getOptions(
            this.chartObject,
            this.chartType,
            this.service,
            this.translate,
    ...
    protected loadLineChart(
        unit: ChronoUnit.Type,
        chartType: typeof this.chartType = this.chartType,
    ) {
        return new Promise<void>((resolve) => {
            Promise.all([
                ...
                this.queryHistoricTimeseriesEnergy(
                    this.service.historyPeriod.value.from,
                    this.service.historyPeriod.value.to,
                ),
            ])
                .then(([dataResponse, energyResponse]) => {
                    this.chartType = "line";
                    dataResponse = DateTimeUtils.normalizeTimestamps(
                        unit,
                        dataResponse,
                    );
                    this.chartObject = this.getChartData();
        ...
        return new Promise((resolve) => {
            Promise.all([
                ...
                .then(([energyPeriodResponse, energyResponse]) => {
                    ...
                    energyPeriodResponse = DateTimeUtils.normalizeTimestamps(
                        unit,
                        energyPeriodResponse,
                    );
    ...
    private getChannelAddresses(): Promise<{
        powerChannels: ChannelAddress[];
        ...
        return new Promise<{
            ...
            if (this.chartObject?.input) {
                resolve({
                    powerChannels: ArrayUtils.sanitize(
                        this.chartObject.input.map(
                            (element) => element.powerChannel,
                        ),
                    ),
                    energyChannels: ArrayUtils.sanitize(
                        this.chartObject.input.map(
                            (element) => element.energyChannel,
                        ),
                    ),
                });
            }
