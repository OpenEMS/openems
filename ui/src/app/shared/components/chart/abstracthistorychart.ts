...
export abstract class AbstractHistoryChart implements OnInit, OnDestroy, AfterViewInit, AfterContentInit {
    ...
    protected channelData: { data: HistoryUtils.ChannelData } = {
        data: {},
    };
     ...
     * @param energyPeriodResponse The response of a {@link QueryHistoricTimeseriesEnergyPerPeriodRequest} or
    ...
    public static fillChart(
        ...
        chartObject.input?.forEach((element) => {
            ...
            if (channelAddress?.toString() in result.data) {
                channelData.data[element.name] =
                    HistoryUtils.CONVERT_WATT_TO_KILOWATT_OR_KILOWATTHOURS(result.data[channelAddress.toString()])?.map(
                        (value) => {
                            if (value == null) {
                                return null;
                            }
        ...
        const displayValues: HistoryUtils.DisplayValue<HistoryUtils.CustomOptions>[] = chartObject.output(
            channelData.data,
            labels,
        );
        const legendOptions: {
        ...
        displayValues.forEach((displayValue, index) => {
            ...
            const yAxis = chartObject.yAxes.find(
                (yaxis) => yaxis?.yAxisId == (displayValue?.yAxisId ?? chartObject.yAxes[0]?.yAxisId),
            );
        ...
        const dataset: Chart.ChartDataset = {
            ...
            yAxisID:
                element.yAxisId != null
                    ? element.yAxisId
                    : chartObject.yAxes.find((element) => element.yAxisId == ChartAxis.LEFT)?.yAxisId,
    ...
    public static getOptions(
        ...
        let options: Chart.ChartOptions = Utils.deepCopy(
            <Chart.ChartOptions>(
                Utils.deepCopy(AbstractHistoryChart.getDefaultXAxisOptions(chartOptionsType, service, labels))
            ),
        );
        const displayValues: HistoryUtils.DisplayValue<HistoryUtils.CustomOptions>[] = chartObject.output(
            channelData.data,
        ...
        options.plugins.tooltip.callbacks.label = (item: Chart.TooltipItem<any>) => {
            ...
            const unit =
                chartObject.yAxes?.find((el) => el.yAxisId === displayValue.yAxisId)?.unit ??
            ...
            return AbstractHistoryChart.getToolTipsSuffix(
                label,
                value,
                displayValue.custom?.formatNumber ?? chartObject.tooltip.formatNumber,
                unit,
                chartType,
                translate,
                config,
        ...
        options.plugins.legend.labels.generateLabels = function (chart: Chart.Chart) {
            ...
            chart.data.datasets.forEach((dataset: Chart.ChartDataset, index) => {
                ...
                const currentDisplayValue = displayValues.find(
                    (el) => el.name == chartLegendLabelItem.text?.split(":")?.[0],
                );

                if (currentDisplayValue?.custom) {
                    const show = !chartLegendLabelItem.hidden;
                    // Hide plugin features on label generate
                    chart.options = AbstractHistoryChart.activateOrDeactivatePlugin(
                        currentDisplayValue,
                        options,
                        chartType,
                        show,
        ...
        options.plugins.tooltip.callbacks.afterTitle = function (items: Chart.TooltipItem<any>[]) {
            ...
            const afterTitle =
                typeof chartObject.tooltip?.afterTitle == "function" ? chartObject.tooltip?.afterTitle(stack) : null;

            const cumulatedValue = datasets
                .filter((el) => el.stack == stack)
                .reduce((_total, dataset) => Utils.addSafely(_total, Math.abs(dataset.data[datasetIndex])), 0);
            const unit = chartObject.yAxes?.find((el) => el.yAxisId === yAxisId)?.unit ?? chartObject.yAxes[0]?.unit;
            ...
            if (afterTitle) {
                return (
                    afterTitle +
                    ": " +
                    formatNumber(cumulatedValue, locale, chartObject.tooltip.formatNumber) +
                    " " +
                    tooltipsLabel
                );
            }
        ...
        options.scales.x.ticks.color = getComputedStyle(document.documentElement).getPropertyValue(
        ...
        switch (element.unit) {
            ...
            case YAxisType.ENERGY:
                options.scales[element.yAxisId] = {
                    ...baseConfig,
                    max:
                        baseConfig?.max && baseConfig?.min && baseConfig?.max === baseConfig?.min
                            ? baseConfig.max + 1
                            : baseConfig.max,
                };
                break;
    ...
    public static getToolTipsSuffix(
        ...
        switch (title) {
            ...
            case YAxisType.RESTRICTION:
            case YAxisType.PERCENTAGE:
                suffix = AbstractHistoryChart.getToolTipsAfterTitleLabel(title, chartType, value, translate);
                break;
            case YAxisType.VOLTAGE:
                suffix = "V";
                break;
     ...
     */
    protected static removePlugins(options: Chart.ChartOptions): Chart.ChartOptions {
    ...
    private static activateOrDeactivatePlugin(
        ...
        switch (displayValue.custom["pluginType"]) {
            case "box":
                if ((displayValue.custom as HistoryUtils.BoxCustomOptions).annotations.length > 0) {
                    options.plugins["annotation"] = {
                        annotations: (displayValue.custom as HistoryUtils.BoxCustomOptions).annotations.map(
                            (annotation) => ({
                                ...annotation,
                                ...AbstractHistoryChart.getColors(displayValue.color, chartType),
                                display: show,
                            }),
                        ),
                    };
                ...
                break;
            case "datalabels":
                options.plugins["datalabels"] = ChartConstants.Plugins.BAR_CHART_DATALABELS(
                    (displayValue.custom as HistoryUtils.DataLabelsCustomOptions).datalabels.displayUnit,
                    true,
                );
                    ...
                    edge.sendRequest(this.service.websocket, request)
                        .then((response) => {
                            ...
                            if (Object.keys(result).length !== 0) {
                                ...
                                this.errorResponse = new JsonrpcResponseError(request.id, {
                                    code: 1,
    ...
    /**
     * Sends the Historic Timeseries Energy per Period Query and makes sure the result is not empty. Symbolizes First
     * substracted from last Datapoint for each period, only used for cumulated channel
     *
     * @param fromDate The From-Date
     * @param toDate The To-Date
     */
    protected queryHistoricTimeseriesEnergyPerPeriod(
        fromDate: Date,
        toDate: Date,
        resolution: ReturnType<typeof calculateResolution>["resolution"] = calculateResolution(
            this.service,
            fromDate,
            toDate,
        ).resolution,
        ...
        const result: Promise<QueryHistoricTimeseriesEnergyPerPeriodResponse> =
            new Promise<QueryHistoricTimeseriesEnergyPerPeriodResponse>((resolve, reject) => {
                this.service.getCurrentEdge().then((edge) => {
                    this.service.getConfig().then(async () => {
                        const channelAddresses = (await this.getChannelAddresses()).energyChannels.filter(
                            (element) => element != null,
                        );
                        const request = new QueryHistoricTimeseriesEnergyPerPeriodRequest(
                            DateUtils.maxDate(fromDate, edge?.firstSetupProtocol),
                            toDate,
                            channelAddresses,
                            resolution,
                        );
                            ...
                            edge.sendRequest(this.service.websocket, request)
                                .then((response) => {
                                    ...
                                    if (Object.keys(result).length != 0) {
                                        ...
                                        resolve(
                                            new QueryHistoricTimeseriesEnergyPerPeriodResponse(response.id, {
                                                timestamps: [],
                                                data: {},
                                            }),
                                        );
    ...
    protected queryHistoricTimeseriesEnergy(
        ...
        const result: Promise<QueryHistoricTimeseriesEnergyResponse> =
            new Promise<QueryHistoricTimeseriesEnergyResponse>((resolve, reject) => {
                this.service.getCurrentEdge().then((edge) => {
                    this.service.getConfig().then(async () => {
                        const channelAddresses =
                            (await this.getChannelAddresses()).energyChannels?.filter((element) => element != null) ??
                            [];
                            ...
                            edge.sendRequest(this.service.websocket, request)
                                .then((response) => {
                                    ...
                                    if (Object.keys(result).length != 0) {
                                        ...
                                        resolve(
                                            new QueryHistoricTimeseriesEnergyResponse(response.id, {
                                                data: { null: null },
                                            }),
                                        );
                                    }
