...
export class AppHeaderComponent implements OnInit, OnDestroy, AfterViewChecked {
    ...
    constructor(
        ...
        effect(() => {
            ...
            const _currentUrl = ROUTE_SERVICE.CURRENT_URL();

            if (currentNode && CURRENT_NODE.GET_PARENTS() && CURRENT_NODE.GET_PARENTS().length > 0) {
    ...
    updateUrl(url: string) {
        ...
        THIS.IS_HEADER_ALLOWED = THIS.IS_ALLOWED_FOR_VIEW(url);
    }
    ...
    updateEnableSideMenu(url: string) {
        ...
        if (file == "user" || file == "settings" || file == "changelog" || file == "login" || file == "index" || URL_ARRAY.LENGTH > 3) {
            ...
            if (THIS.NAVIGATION_SERVICE.POSITION() != "disabled") {
    ...
    public segmentChanged(event) {
        if (EVENT.DETAIL.VALUE == "IndexLive") {
            ...
            THIS.ROUTER.NAVIGATE(["/device/" + THIS.SERVICE.CURRENT_EDGE().id + "/live"], { replaceUrl: true });
            // THIS.ROUTER.NAVIGATE_BY_URL("/device/" + THIS.SERVICE.CURRENT_EDGE().id + "/live", { replaceUrl: true });
            THIS.CD_REF.DETECT_CHANGES();
        }
        if (EVENT.DETAIL.VALUE == "IndexHistory") {

            THIS.ROUTER.NAVIGATE(["/device/" + THIS.SERVICE.CURRENT_EDGE().id + "/history"]);
            /** Creates bug of being infinite forwarded betweeen live and history, if not relatively routed  */
            // THIS.ROUTER.NAVIGATE_BY_URL("/device/" + THIS.SERVICE.CURRENT_EDGE().id + "/history", { replaceUrl: true });
            THIS.CD_REF.DETECT_CHANGES();
        }
    }
