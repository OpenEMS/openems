 ...
 *
 */
public class OpenemsWebsocketServer extends WebSocketServer {

	private final Logger log = LoggerFactory.getLogger(OpenemsWebsocketServer.class);
	private final BiMap<WebSocket, Integer> websockets = Maps.synchronizedBiMap(HashBiMap.create());

	protected OpenemsWebsocketServer(int port) throws Exception {
	...
	public void onOpen(WebSocket websocket, ClientHandshake handshake) {
		try {
			if (handshake.hasFieldValue("apikey")) {
				...
				try {
					Optional<Device> deviceOpt = OdooProvider.getInstance().getDeviceModel().getDeviceForApikey(apikey);
					if (!deviceOpt.isPresent()) {
						throw new FemsException("Unable to find device for apikey [" + apikey + "]");
					}
					Device device = deviceOpt.get();
					this.websockets.put(websocket, device.getId());
	...
	public void onClose(WebSocket websocket, int code, String reason, boolean remote) {
		log.info("Close connection to [" + this.websockets.get(websocket) + "]" //
	...
	private void timedata(Device device, JsonElement jTimedataElement) {
		try {
			...
			// Write to InfluxDB
			try {
				InfluxdbProvider.getInstance().write(device.getNameNumber(), jTimedata);
	...
	private Optional<Device> getDevice(WebSocket websocket) {
		Integer id = this.websockets.get(websocket);
		return OdooProvider.getInstance().getDeviceCache().getDeviceForId(id);
	}
