...
public class PlcNextGdsProvider {
	...
	private final PlcNextGdsDataClient dataClient;
	
	@Activate
	public PlcNextGdsProvider(@Reference(scope = ReferenceScope.BUNDLE) PlcNextGdsDataClient dataClient) {
	 ...
	 */
	public void readFromApiToChannels(PlcNextGdsDataClientConfig config) {
		...
		List<ChannelId> gdsDataRedableAspectChannelIds = gdsDataReadableAspects.stream() //
				...
				.toList();
		Map<ChannelId, Channel<?>> mappedChannels = config.channels().parallelStream() //
		...
		log.info("Fetching GDS data for instance '" + config.dataInstanceName() + "'");
		JsonObject apiResponseBody = dataClient.fetchAllGdsDataAspects(config);

		log.info("Processing data and filling channels of instance '" + config.dataInstanceName() + "'");
