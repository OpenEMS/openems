...
*/
	...
	private void setLimits() throws OpenemsNamedException {
		...
		if (this.config.readOnlyMode()) {
			...
			EnumWriteChannel setChargeDischargeDefault	= this.channel(SolarEdgeHybridEss.ChannelId.SET_CHARGE_DISCHARGE_DEFAULT_MODE);
			...
			setChargeDischargeDefault.setNextWriteValue(ChargeDischargeMode.SE_CHARGE_POLICY_MAX_SELF_CONSUMPTION);	// Always allowed.When used with Maximize self-consumption, only excess power is used for charging (charging from the grid is not allowed)
		
			setChargePowerLimit.setNextWriteValue((this.config.ChargePowerLimit() * -1));
			setDischargePowerLimit.setNextWriteValue(this.config.DischargePowerLimit());
	...
	public String debugLog() {
		return "SoC:" + this.getSoc().asString() //
				...
				+ this.channel(SolarEdgeHybridEss.ChannelId.MAX_CHARGE_POWER).value().asStringWithoutUnit() + " / " + this.channel(SolarEdgeHybridEss.ChannelId.MAX_CHARGE_PEAK_POWER).value().asStringWithoutUnit() + ";"
				+ "|Allowed DisCharge Power/Peak:"
				+ this.channel(SolarEdgeHybridEss.ChannelId.MAX_DISCHARGE_POWER).value().asStringWithoutUnit() + " / " + this.channel(SolarEdgeHybridEss.ChannelId.MAX_DISCHARGE_PEAK_POWER).value().asStringWithoutUnit() + ";"
