...
public class ControllerIoHeatingElementImpl extends AbstractOpenemsComponent
	...
	public ControllerIoHeatingElementImpl() throws OpenemsNamedException {
		super(//
	...
	public void run() throws OpenemsNamedException {
		...
		// Update Channels
		this.channel(ControllerIoHeatingElement.ChannelId.STATUS).setNextValue(runState);
		this.channel(ControllerIoHeatingElement.ChannelId.PHASE1_TIME).setNextValue(phase1Time);
		this.channel(ControllerIoHeatingElement.ChannelId.PHASE2_TIME).setNextValue(phase2Time);
		this.channel(ControllerIoHeatingElement.ChannelId.PHASE3_TIME).setNextValue(phase3Time);
		this.channel(ControllerIoHeatingElement.ChannelId.TOTAL_PHASE_TIME).setNextValue(totalPhaseTime);

		this.channel(ControllerIoHeatingElement.ChannelId.LEVEL1_TIME).setNextValue(phase1Time - phase2Time);
		this.channel(ControllerIoHeatingElement.ChannelId.LEVEL2_TIME).setNextValue(phase2Time - phase3Time);
		this.channel(ControllerIoHeatingElement.ChannelId.LEVEL3_TIME).setNextValue(phase3Time);
	...
	protected Status modeAutomatic() throws IllegalArgumentException, OpenemsNamedException {
		...
		if (this.config.workMode().equals(WorkMode.TIME)) {
			if (now.isAfter(configuredEndTime) || latestForceChargeStartTime == null) {
				this.channel(ControllerIoHeatingElement.ChannelId.FORCE_START_AT_SECONDS_OF_DAY).setNextValue(null);
			} else {
				...
				this.channel(ControllerIoHeatingElement.ChannelId.FORCE_START_AT_SECONDS_OF_DAY)
	...
	public void applyLevel(Level level) throws IllegalArgumentException, OpenemsNamedException {
		// Update Channel
		this.channel(ControllerIoHeatingElement.ChannelId.LEVEL).setNextValue(level);
	...
	private Level applyHysteresis(Level targetLevel) {
		if (this.currentLevel != targetLevel) {
			...
			if (this.lastLevelChange.plus(hysteresis).isBefore(now)) {
				...
				this.channel(ControllerIoHeatingElement.ChannelId.AWAITING_HYSTERESIS).setNextValue(false);
			} else {
				// wait for hysteresis
				this.channel(ControllerIoHeatingElement.ChannelId.AWAITING_HYSTERESIS).setNextValue(true);
			}
		} else {
