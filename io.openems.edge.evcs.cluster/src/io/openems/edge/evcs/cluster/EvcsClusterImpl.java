...
public class EvcsClusterImpl extends AbstractOpenemsComponent
	...
	private final Logger log = LoggerFactory.getLogger(EvcsClusterImpl.class);
	...
	protected void limitEvcss() {
		...
		this.channel(EvcsCluster.ChannelId.MAXIMUM_POWER_TO_DISTRIBUTE).setNextValue(Integer.valueOf(totalPowerLimit));
	...
	public int getMaximumPowerToDistribute() {
		...
		//TKL
		int maxAvailableStoragePower =0; //TKL
		if (!config.ess_id().equals("NONE") ) 
		{
