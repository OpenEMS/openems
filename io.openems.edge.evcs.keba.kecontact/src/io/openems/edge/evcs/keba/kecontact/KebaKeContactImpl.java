...
public class KebaKeContactImpl extends AbstractManagedEvcsComponent
		implements KebaKeContact, ManagedEvcs, Evcs, OpenemsComponent, EventHandler, ModbusSlave {
	...
	public KebaKeContactImpl() {
		super(//
				...
				KebaKeContact.ChannelId.values() //
		);
	}
	...
	protected boolean send(String s) {
		var raw = s.getBytes();
		var packet = new DatagramPacket(raw, raw.length, this.ip, KebaKeContactImpl.UDP_PORT);
	...
	private void resetChannelValues() {
		for (var c : KebaKeContact.ChannelId.values()) {
