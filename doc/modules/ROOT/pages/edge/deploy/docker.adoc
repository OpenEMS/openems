...
4. Updated environment variables (`WEBSOCKET_HOST` and `WEBSOCKET_PORT`) replacing the deprecated `UI_WEBSOCKET`

NOTE: The `UI_WEBSOCKET` variable has been deprecated. Use `WEBSOCKET_HOST` and `WEBSOCKET_PORT` instead. This configuration allows the UI to connect to the OpenEMS Edge Websocket. Ensure the hostname is reachable from any device you want to use the UI on.
