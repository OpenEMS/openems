...
----
. Eclipse will underline `ElectricityMeter` and show you the error *ElectricityMeter cannot be resolved to a type*. Resolve it by importing adding an `import io.openems.edge.meter.api.ElectricityMeter;`.
+
NOTE: The easiest way to fix these kind of import errors is to to selecto btn:[Source] → btn:[Organize Imports] in the menu or simply press btn:[Ctrl] + btn:[Shift] + btn:[o]. Alternatively click the 'error light bulb' next to the line with the error and select btn:[Import 'ElectricityMeter' (io.openems.edge.meter.api)].
...
The Device that we are is a *ElectricityMeter*. We already defined the required Channels in the _initializeChannels()_ method. Additionally the Component also needs to implement the Nature interface.
+
