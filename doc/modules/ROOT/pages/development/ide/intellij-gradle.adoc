...
This page describes how to import the OpenEMS *fems* repository into IntelliJ IDEA as a Gradle project and how to create useful run/debug configurations.

NOTE: This guide intentionally ignores screenshots. Replace the `IMAGE PLACEHOLDER` blocks with real screenshots when publishing.
...
. Choose the repository folder (commonly named `fems`) and import it as a *Gradle* project.
...
. Add a *Gradle* task.
. Set:
** *Gradle project*: `fems` (sometimes listed under a parent like `openems`)
** *Tasks*: `buildEdge`
...
. Configure the thresholds for wildcard imports to high values (example: `99`) to avoid `*` imports:
