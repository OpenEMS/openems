...
NOTE: The bars in the gauge are not based on the current power distribution but on the maximum values ever recorded for each source.
So they do not represent the actual live ratio between grid, PV, and consumption. Instead, they show how the current values compare to their respective historical maximums.
