...
NOTE: If you plan to actively develop on OpenEMS UI, you can now also setup a development environment for it using xref:ui/setup-ide.adoc[this guide]. Otherwise you can xref:ui/run-prebuilt.adoc[use the prebuilt UI] or xref:ui/deploy.adoc[deploy the OpenEMS UI as docker image].

. Make sure OpenEMS Edge is running locally and the port of the websocket matches the port the UI is expecting. For the prebuilt UI it is port `8075`.

. Make sure the webserver for the UI is running.

. Open the UI URL, e.g. http://localhost:4200. IF the UI is running on a remote machine, replace `localhost` by the IP address of that machine.
