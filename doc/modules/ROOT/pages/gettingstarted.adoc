...
== Setup Intellij IDE for OpenEMS

. Prepare Intellij IDE
...
.. We use the Amdatu Plugin. So first check if it is working with the newest Version, so check the homepage https://plugins.jetbrains.com/plugin/10639-amdatu/versions[Amdatu schauen icon:external-link[]].
...
.. You can even disable in the Toolbox auto-Updates, also you can select a previous if the update was done accidently
.. Search under Plugins "Amdatu" und "Checkstyle" suchen
+
.intellij settings
...
+
Rightclick on a existing modul: New -> Module
+
...
.. If there is the module not right loaded, you can see that the blue square is not present
+
.not loaded modules
image::intellij-load-modul.png[]
+
That get be fixed by Tools->Amdatu->Refresh-Workspace. If it is still not working, File->Invalidate Caches/Restart
