...
image:eclipse-io.openems.backend.application.png[alt="io.openems.backend.application in eclipse project"]
.. Start the Backend App
image:eclipse-backendapp-bndrund.png[alt="Start the Backend Application in Eclipse"]
.. Open the Felix Web interface of the Backend: http://localhost:8079/system/console/configMgr
Username: admin, Password: admin
... Select the `EdgeWebsocket` Bundle and specify the Port: 8084
image:config-backend-edgeWebsocket.png[alt="Start the EdgeWebsocket to listen on port 8084"]
.. Start the clienet connection form the `Edge App` in the Felix Web interface. The uri has to be changed to the backend address and port: ws://localhost:8084
image:config-edge-api-backend.png[alt="Connect to Edge App to the Backend Application Server"]
. Install InfluxDB and Chronograf: https://www.influxdata.com
It is recommended from the Influx Developers to use a Unix system such as Liunx, thuns support for Windows is experimental: https://www.influxdata.com/blog/running-the-tick-stack-on-windows/
...
image:config-edge-influxdb.png[alt="Configure the Edge Influx Connection"]
...
image:config-backend-influxdb.png[alt="Configure the Backend Influx Connection"]
